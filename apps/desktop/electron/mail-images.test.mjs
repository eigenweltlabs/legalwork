...
test('native image download pins checked DNS, blocks redirect-to-private and sends no ambient credentials',async()=>{
 ...
 const transport=()=>function(url,options,callback){requests++;assert.equal(options.headers.Cookie,undefined);assert.equal(options.headers.Authorization,undefined);assert.equal(options.headers.Referer,undefined);assert.equal(options.agent,false);options.lookup(url.hostname,{},(_error,address)=>assert.equal(address,'8.8.8.8'));const request=Object.assign(new EventEmitter(),{setTimeout:()=>{},destroy(error){request.emit('error',error);},end(){const response=Object.assign(Readable.from([]),{statusCode:302,headers:{location:'https://internal.example.test/secret'}});callback(response);}});return request;};
...
test('encoded dimensions reject bombs before raster decode',()=>{assert.deepEqual(rasterDimensions(png),{width:72,height:40,type:'png',frames:1,pixels:2880});const bomb=Buffer.from(png);bomb.writeUInt32BE(100000,16);assert.throws(()=>rasterDimensions(bomb),/dimensions/);assert.throws(()=>rasterDimensions(Buffer.from('<svg/>')),/dimensions/);});
test('native image grants read the current stored body, bound returned bytes, and reject caller URLs or a lock',async()=>{
 ...
 const server=createServer(async(req,res)=>{assert.equal(req.headers['x-legalwork-host-token'],'synthetic');if(locked){res.writeHead(423);res.end();return;}let text='';for await(const data of req)text+=data;const input=JSON.parse(text),offset=input.request.offset,bytes=body.subarray(offset,offset+input.request.limit);res.setHeader('Content-Type','application/json');res.end(JSON.stringify({accountId:'archive',offset,totalBytes:body.length,referenceId:'sha256:'+hash,sha256:hash,data:bytes.toString('base64'),nextOffset:offset+bytes.length===body.length?null:offset+bytes.length}));});await new Promise(resolve=>server.listen(0,'127.0.0.1',()=>resolve(undefined)));
 const address=server.address();assert(address&&typeof address!=='string');
 const api=createMailImages({connection:async()=>({running:true,baseUrl:'http://127.0.0.1:'+address.port,hostToken:'synthetic'}),decode:()=>({width:72,height:40}),download:async url=>{calls++;assert.equal(url,'https://images.example.test/a');return png;}}),input={accountId:'archive',locator:{provider:'archive',namespace:'synthetic',entryId:'one'},referenceId:'sha256:'+hash,partId:''};
...
test('native transport reads bounded raster bytes and rejects oversized or encoded responses',async()=>{
 ...
 const transport=(chunks,headers={})=>()=>function(_url,_options,callback){const request=Object.assign(new EventEmitter(),{setTimeout:()=>{},destroy(error){request.emit('error',error);},end(){const response=Object.assign(Readable.from(chunks),{statusCode:200,headers});callback(response);}});return request;};
...
test('aggregate decoded pixels stop before another compressed image reaches the decoder',async()=>{
 ...
 const server=createServer(async(req,res)=>{let text='';for await(const data of req)text+=data;const offset=JSON.parse(text).request.offset,limit=JSON.parse(text).request.limit,bytes=body.subarray(offset,offset+limit);res.setHeader('Content-Type','application/json');res.end(JSON.stringify({accountId:'a',offset,totalBytes:body.length,referenceId:'sha256:'+hash,sha256:hash,data:bytes.toString('base64'),nextOffset:offset+bytes.length===body.length?null:offset+bytes.length}));});await new Promise(resolve=>server.listen(0,'127.0.0.1',()=>resolve(undefined)));
 const address=server.address();assert(address&&typeof address!=='string');
 const api=createMailImages({connection:async()=>({running:true,baseUrl:'http://127.0.0.1:'+address.port,hostToken:'synthetic'}),decode:()=>{decoded++;return {width:2048,height:2048};},download:async()=>large});
