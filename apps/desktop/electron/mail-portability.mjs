...
export async function performMailPortability(value,dialog){
 ...
 const expected=operation==='abandon'?['operation','id','confirmation']:operation==='list'?['operation']:operation==='pause'||operation==='resume'?['operation','id']:operation==='import'?['operation','format']:operation==='export'?['operation','format','accountId']:[];
 ...
 if(operation==='abandon'){if(typeof value.id!=='string'||!/^[0-9a-f-]{36}$/.test(value.id)||value.confirmation!=='ABANDON CHECKPOINT')throw Error('Invalid archive job');return target.portability({operation:'mail.portability.abandon',id:value.id,confirmation:value.confirmation});}
