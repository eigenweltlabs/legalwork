...
test('native reply drops route/focus changes and never invents fallback after errors',async()=>{
 let url='https://fixture.test/mail';
 /** @type {(value:unknown)=>void} */ let resolve;const contents={getURL:()=>url,isDestroyed:()=>false,executeJavaScript:()=>new Promise(done=>{resolve=done;})};
 const pending=dispatchMailReply(contents,false);url='https://fixture.test/settings';assert.ok(resolve);resolve('unused');assert.equal(await pending,'expired');
 const focus=dispatchMailReply(contents,true,()=>false);assert.ok(resolve);resolve('unused');assert.equal(await focus,'expired');
...
test('native queued dispatch checks renderer focus before opening and coalesces accelerators',async()=>{
 let code,executions=0;
 /** @type {(value:unknown)=>void} */ let resolve;const contents={getURL:()=> 'https://fixture.test/mail',isDestroyed:()=>false,executeJavaScript:value=>{executions++;code=value;return new Promise(done=>{resolve=done;});}};
 ...
 const result=vm.runInNewContext(code,{Date,location:{href:contents.getURL()},document:{hasFocus:()=>false,get activeElement(){throw Error('must not dispatch');}}});assert.equal(result,'expired');assert.ok(resolve);resolve(result);assert.equal(await first,'expired');
});
