...
export { isDesktopRuntime, isElectronRuntime, isOfficeAddinRuntime } from "../lib/runtime-env";
...
const STARTUP_PREF_KEY = "legalwork.startupPref";
const LEGACY_PREF_KEY = "legalwork.modePref";
const LEGACY_PREF_KEY_ALT = "legalwork_mode_pref";
...
export function getWorkspaceTaskLoadErrorDisplay(workspace: WorkspaceInfo, error?: string | null) {
  ...
  const host = `${workspace.baseUrl ?? ""} ${workspace.legalworkHostUrl ?? ""}`.toLowerCase();
...
const TRUNCATED_SYSTEM_PATH_PATTERNS = [
  /com\.[^/]+\.(legalwork|opencode)/i, // macOS app bundle identifiers
  /\.legalwork\.dev\//i, // LegalWork dev paths
