...
export function AuthorizedFoldersPanel(props: AuthorizedFoldersPanelProps) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const response = await legalworkClient.listAuthorizedFolders(legalworkWorkspaceId);
        if (cancelled) return;
  ...
  const persistAuthorizedFolders = useCallback(async (nextFolders: string[]) => {
    ...
    try {
      const response = await legalworkClient.setAuthorizedFolders(legalworkWorkspaceId, nextFolders);
