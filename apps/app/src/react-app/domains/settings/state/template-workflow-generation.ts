...
function buildGenerationPrompt(existingWorkflowNames: string[]): string {
  ...
  return [
    ...
    skipList,
    `Open and read each template (step 3) before you decide anything about it: you name and skip based on what the document is, not on its file name.`,
    `SKIP a template when one of the workflows above already covers the same document. The names above are meaningful, so match on meaning rather than exact spelling. Never create a duplicate or a second variant of an existing workflow.`,
    ``,
    ...
    `- Name the workflow yourself, from what the document IS rather than from its file name. Its folder is "workflow-assistant-<slug>", where you choose <slug> to capture the document type and its distinguishing subject in a few words (a building-permit application becomes "workflow-assistant-antrag-baugenehmigung", NOT the raw file name). The slug must be: lowercase ASCII kebab-case only (a-z, 0-9, single hyphens) with accents and umlauts transliterated (ä→ae, ö→oe, ü→ue, ß→ss); written in the template's own language so it reads naturally to the firm's lawyers; two to five meaningful words with filler dropped (articles and prepositions like "der"/"auf", boilerplate like "vorlage"/"muster"/"template", and internal form numbers unless the firm actually refers to the document by that number). The whole folder name must be at most ${MAX_SKILL_NAME_LENGTH} characters. This is a hard limit: each workflow is exposed to the assistant as a tool, and the model rejects tool names longer than that, so a longer name would break the workflow when it runs. If your name runs long, drop the least important words rather than cutting mid-word. (LegalWork will also shorten an over-long name on import as a safety net, but a name you keep within budget yourself stays the most readable, so aim to fit.) Each workflow needs a distinct name: if two templates would land on the same slug, add the detail that tells them apart. The name should read well when shown title-cased (e.g. "Antrag Baugenehmigung") and tell a lawyer at a glance what the workflow drafts.`,
