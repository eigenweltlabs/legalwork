...
import {
  ...
  openDesktopUrl,
  legalworkServerInfo as legalworkServerInfoCmd,
  legalworkServerRestart as legalworkServerRestartCmd,
  pickFile,
...
import type { LegalworkServerStore, LegalworkServerStoreSnapshot } from "../../connections/legalwork-server-store";

const STARTUP_PREFERENCE_KEY = "legalwork.startupPreference";
const ENGINE_SOURCE_KEY = "legalwork.engineSource";
const ENGINE_CUSTOM_BIN_KEY = "legalwork.engineCustomBinPath";
const OPENCODE_ENABLE_EXA_KEY = "legalwork.opencodeEnableExa";

type ResetModalMode = "onboarding" | "all";

const ONBOARDING_LOCAL_STORAGE_KEYS = [
  "legalwork.acknowledgedProviders",
  "legalwork.orgOnboardingSeen",
  "legalwork.reloadAfterOrgOnboarding",
  "legalwork.seenProviderIds",
];

type UseDebugViewModelOptions = {
  developerMode: boolean;
  legalworkServerStore: LegalworkServerStore;
  legalworkServerSnapshot: LegalworkServerStoreSnapshot;
...
function clearLegalworkLocalStorageForReset(mode: ResetModalMode): void {
  if (typeof window === "undefined") return;
  try {
    if (mode === "all") {
    ...
    const raw = window.localStorage.getItem("legalwork.preferences");
    if (raw) {
      const prefs = JSON.parse(raw);
      prefs.hasCompletedOnboarding = false;
      window.localStorage.setItem("legalwork.preferences", JSON.stringify(prefs));
    }
  } catch {
    // ignore persistence failures
  }
...
function formatManagedOpencodeBinary(info: LegalworkServerInfo | null) {
...
function describeLegalworkServer(info: LegalworkServerInfo | null) {
  const running = Boolean(info?.running);
  return {
    ...statusPill(running),
    lines: [
...
export function useDebugViewModel(options: UseDebugViewModelOptions) {
  ...
  const [legalworkServerRestarting, setLegalworkServerRestarting] = useState(false);
  const [opencodeServiceStatus, setOpencodeServiceStatus] = useState<{
    tone: "success" | "error";
    message: string;
  } | null>(null);
  const [legalworkServiceStatus, setLegalworkServiceStatus] = useState<{
    tone: "success" | "error";
    message: string;
  } | null>(null);
  const [opencodeLogStatus, setOpencodeLogStatus] = useState<string | null>(null);
  const [legalworkLogStatus, setLegalworkLogStatus] = useState<string | null>(null);
  ...
  const runtimeSummary = useMemo(
    () => ({
      ...
      legalworkServerVersionLabel: legalworkServerSnapshot.legalworkServerDiagnostics?.version ?? "—",
    }),
    [
      appBuild?.gitSha,
      appBuild?.version,
      engineInfoState?.baseUrl,
      legalworkServerSnapshot.legalworkServerDiagnostics?.version,
    ],
  );
  ...
  const runtimeDebugReport = useMemo(() => {
    return {
      ...
      legalworkServer: {
        hostInfo: legalworkServerSnapshot.legalworkServerHostInfo,
        diagnostics: legalworkServerSnapshot.legalworkServerDiagnostics,
        capabilities: legalworkServerSnapshot.legalworkServerCapabilities,
        settings: legalworkServerSnapshot.legalworkServerSettings,
        status: legalworkServerSnapshot.legalworkServerStatus,
        url: legalworkServerSnapshot.legalworkServerUrl,
      },
      runtimeWorkspaceId,
      selectedWorkspaceRoot,
    };
    ...
    appBuild,
    engineInfoState,
    legalworkServerSnapshot.legalworkServerCapabilities,
    legalworkServerSnapshot.legalworkServerDiagnostics,
    legalworkServerSnapshot.legalworkServerHostInfo,
    legalworkServerSnapshot.legalworkServerSettings,
    legalworkServerSnapshot.legalworkServerStatus,
    legalworkServerSnapshot.legalworkServerUrl,
    runtimeWorkspaceId,
    selectedWorkspaceRoot,
  ]);
  ...
  const legalworkCard = useMemo(
    () => describeLegalworkServer(legalworkServerSnapshot.legalworkServerHostInfo),
    [legalworkServerSnapshot.legalworkServerHostInfo],
  );
  ...
  const onExportRuntimeDebugReport = useCallback(async () => {
    try {
      downloadTextAsFile(
        `legalwork-runtime-${new Date().toISOString().replace(/[:.]/g, "-")}.json`,
        runtimeDebugReportJson,
        "application/json",
      );
  ...
  const onExportDeveloperLog = useCallback(async () => {
    try {
      downloadTextAsFile(
        `legalwork-developer-${new Date().toISOString().replace(/[:.]/g, "-")}.log`,
        developerLog.join("\n"),
        "text/plain",
      );
  ...
  const onRevealElectronMigrationBackup = useCallback(async () => {
    ...
    try {
      ...
      if (!appBundlePath) {
        setElectronMigrationStatus("Could not resolve the current LegalWork.app bundle path.");
        return;
      }
      await revealDesktopItemInDir(`${appBundlePath}.migrate-bak`);
      setElectronMigrationStatus("Requested Finder reveal for LegalWork.app.migrate-bak. The backup exists after an install handoff completes.");
    } catch (error) {
      setElectronMigrationStatus(error instanceof Error ? error.message : safeStringify(error));
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const bridge = window.__LEGALWORK_ELECTRON__?.updater;
  ...
  const bootFullEngineStack = useCallback(async () => {
    ...
    // Collect ALL local workspace paths so legalwork-server is started with
    ...
    const info = await engineStartCmd(workspacePath, {
      ...
      legalworkRemoteAccess:
        optionsRef.current.legalworkServerSnapshot.legalworkServerSettings
          .remoteAccessEnabled === true,
    });

    // engine_start restarts legalwork-server on a NEW port and lets that server
    // manage OpenCode. Re-read host info and persist the fresh URL/token.
    try {
      const hostInfo = (await legalworkServerInfoCmd()) as {
        baseUrl?: string;
        ownerToken?: string;
        clientToken?: string;
        hostToken?: string;
    ...
    return info;
  }, [legalworkServerStore, refreshEngineInfo]);
  ...
  const onRestartLegalworkServer = useCallback(async () => {
    if (!isDesktopRuntime()) return;
    setLegalworkServerRestarting(true);
    setLegalworkServiceStatus(null);
    setServiceRestartError(null);
    try {
      await legalworkServerRestartCmd({
        remoteAccessEnabled: legalworkServerSnapshot.legalworkServerSettings.remoteAccessEnabled === true,
      });
      ...
      setLegalworkServerRestarting(false);
    }
  }, [
    legalworkServerSnapshot.legalworkServerSettings.remoteAccessEnabled,
    legalworkServerStore,
    pushDeveloperLog,
  ]);
  ...
  const onExportOpencodeLogs = useCallback(async () => {
    ...
    try {
      downloadTextAsFile(
        `legalwork-opencode-${new Date().toISOString().replace(/[:.]/g, "-")}.log`,
        text,
        "text/plain",
      );
  ...
  const onCopyLegalworkLogs = useCallback(async () => {
    const info = legalworkServerSnapshot.legalworkServerHostInfo;
    const text = formatServiceLogs(info?.lastStdout, info?.lastStderr);
    if (!text) {
      setLegalworkLogStatus(t("settings.no_logs_captured"));
      return;
    }
    try {
      await navigator.clipboard.writeText(text);
  ...
  }, [formatServiceLogs, legalworkServerSnapshot.legalworkServerHostInfo]);

  const onExportLegalworkLogs = useCallback(async () => {
    const info = legalworkServerSnapshot.legalworkServerHostInfo;
    const text = formatServiceLogs(info?.lastStdout, info?.lastStderr);
    if (!text) {
      setLegalworkLogStatus(t("settings.no_logs_captured"));
    ...
    try {
      downloadTextAsFile(
        `legalwork-server-${new Date().toISOString().replace(/[:.]/g, "-")}.log`,
        text,
        "text/plain",
      );
      setLegalworkLogStatus(t("settings.exported_developer_log"));
    } catch (error) {
      setLegalworkLogStatus(error instanceof Error ? error.message : safeStringify(error));
    }
  }, [formatServiceLogs, legalworkServerSnapshot.legalworkServerHostInfo]);
  ...
  const onNukeLegalworkAndOpencodeConfig = useCallback(async () => {
    if (!isDesktopRuntime()) return;
    const confirmed =
      typeof window === "undefined"
        ? true
        : window.confirm(
            "Delete ALL local LegalWork + OpenCode config and quit? This cannot be undone.",
          );
    if (!confirmed) return;
    setNukeConfigBusy(true);
    setNukeConfigStatus(null);
    try {
      await nukeLegalworkAndOpencodeConfigAndExit();
    } catch (error) {
  ...
  const debugProps: DebugViewProps = useMemo(
    () => ({
      ...
      startupLabel:
        legalworkServerSnapshot.legalworkServerStatus === "connected"
          ? t("settings.legalwork_server_label")
      ...
      opencodeRestarting,
      legalworkServerRestarting,
      opencodeServiceStatus,
      legalworkServiceStatus,
      opencodeLogStatus,
      legalworkLogStatus,
      onCopyOpencodeLogs,
      onExportOpencodeLogs,
      onCopyLegalworkLogs,
      onExportLegalworkLogs,
      serviceRestartError,
      onRestartOpencode,
      onRestartLegalworkServer,
      engineCard,
      opencodeConnectCard,
      legalworkCard,
      legalworkServerDiagnostics: legalworkServerSnapshot.legalworkServerDiagnostics,
      runtimeWorkspaceId,
      legalworkServerCapabilities: legalworkServerSnapshot.legalworkServerCapabilities,
      pendingPermissions: {},
      events: [],
      workspaceDebugEvents: [],
      safeStringify,
      onClearWorkspaceDebugEvents,
      legalworkAuditEntries: legalworkServerSnapshot.legalworkAuditEntries,
      legalworkAuditStatus: auditStatusPill(legalworkServerSnapshot.legalworkAuditStatus),
      legalworkAuditError: legalworkServerSnapshot.legalworkAuditError,
      opencodeConnectStatus: null,
      opencodeDevModeEnabled: appBuild?.legalworkDevMode === true,
      nukeConfigBusy,
      nukeConfigStatus,
      onNukeLegalworkAndOpencodeConfig,
    }),
    [
      appBuild?.legalworkDevMode,
      developerLog,
      developerLogStatus,
      developerMode,
      electronMigrationBusy,
      ...
      startupStatus,
      workspaceDebugEventsStatus,
      legalworkServerSnapshot.legalworkAuditEntries,
      legalworkServerSnapshot.legalworkAuditError,
      legalworkServerSnapshot.legalworkAuditStatus,
      legalworkServerSnapshot.legalworkServerCapabilities,
      legalworkServerSnapshot.legalworkServerDiagnostics,
      legalworkServerSnapshot.legalworkServerStatus,
      resetModalBusy,
