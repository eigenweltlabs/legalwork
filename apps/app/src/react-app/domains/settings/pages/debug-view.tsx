...
type RuntimeSummary = {
  ...
  legalworkServerVersionLabel: string;
};
...
export type DebugViewProps = {
  ...
  legalworkAuditStatus: StatusPill;
  legalworkAuditError: string | null;
  opencodeConnectStatus: OpencodeConnectStatus | null;
  opencodeDevModeEnabled: boolean;
  nukeConfigBusy: boolean;
  nukeConfigStatus: string | null;
  onNukeLegalworkAndOpencodeConfig: () => void | Promise<void>;
};

function formatActor(entry: LegalworkAuditEntry) {
    ...
    <section className="space-y-6 max-w-3xl w-full">
      ...
      <div className={cardClass}>
        ...
        <div className="grid gap-2 text-[12px] text-dls-secondary md:grid-cols-2">
          ...
          <div>
            {t("settings.debug_legalwork_server_version", {
              version: props.runtimeSummary.legalworkServerVersionLabel,
            })}
          </div>
        </div>
      ...
      <div className={cardClass}>
        ...
        <div className="grid gap-3 grid-cols-1 lg:grid-cols-2">
          <ServiceCard
            title={t("settings.legalwork_server_label")}
            description={t("settings.legalwork_config_sidecar_desc")}
            pill={props.legalworkCard}
            lines={props.legalworkCard.lines}
            stdout={props.legalworkCard.stdout ?? null}
            stderr={props.legalworkCard.stderr ?? null}
            execution={props.legalworkCard.execution ?? null}
            error={props.legalworkCard.error ?? null}
            restarting={props.legalworkServerRestarting}
            restartLabel={t("settings.restart_legalwork_server")}
            onRestart={props.onRestartLegalworkServer}
            serviceStatus={props.legalworkServiceStatus}
            logStatus={props.legalworkLogStatus}
            onCopyLogs={props.onCopyLegalworkLogs}
            onExportLogs={props.onExportLegalworkLogs}
            isDesktop={isDesktop}
          />
      ...
      <div className={cardClass}>
        <div className={sectionHeaderClass}>
          <div className={sectionTitleClass}>{t("settings.legalwork_diagnostics_title")}</div>
          <div className={sectionDescClass}>
            <span className="font-mono text-[11px] text-dls-secondary">
              {props.legalworkServerDiagnostics?.version ?? "—"}
            </span>
          </div>
        </div>

        {props.legalworkServerDiagnostics ? (
          <div className="grid gap-2 text-[12px] text-dls-secondary md:grid-cols-2">
            <div>{t("settings.diag_started", { time: formatUptime(props.legalworkServerDiagnostics.uptimeMs) })}</div>
            <div>
              {t("settings.diag_read_only", {
                value: props.legalworkServerDiagnostics.readOnly ? "true" : "false",
              })}
            </div>
            <div>
              {t("settings.diag_approval", {
                mode: props.legalworkServerDiagnostics.approval.mode,
                ms: String(props.legalworkServerDiagnostics.approval.timeoutMs),
              })}
            </div>
            <div>{t("settings.diag_workspaces", { count: String(props.legalworkServerDiagnostics.workspaceCount) })}</div>
            <div>
              {t("settings.diag_selected_workspace", {
                id: props.legalworkServerDiagnostics.selectedWorkspaceId ?? "—",
              })}
            </div>
            <div>
              {t("settings.diag_runtime_workspace", {
                id: props.legalworkServerDiagnostics.activeWorkspaceId ?? "—",
              })}
            </div>
            <div>
              {t("settings.diag_config_path", {
                path: props.legalworkServerDiagnostics.server.configPath ?? t("settings.diag_default"),
              })}
            </div>
            <div>
              {t("settings.diag_token_source", {
                source: props.legalworkServerDiagnostics.tokenSource.client,
              })}
            </div>
            <div>
              {t("settings.diag_host_token_source", {
                source: props.legalworkServerDiagnostics.tokenSource.host,
              })}
            </div>
          </div>
        ) : (
        ...
        <div className={subCardClass}>
          ...
          {props.legalworkServerCapabilities ? (
            <div className="grid gap-2 text-[12px] text-dls-secondary md:grid-cols-2">
              <div>{t("settings.cap_skills", { value: formatCapability(props.legalworkServerCapabilities.skills) })}</div>
              <div>{t("settings.cap_plugins", { value: formatCapability(props.legalworkServerCapabilities.plugins) })}</div>
              <div>{t("settings.cap_mcp", { value: formatCapability(props.legalworkServerCapabilities.mcp) })}</div>
              <div>{t("settings.cap_commands", { value: formatCapability(props.legalworkServerCapabilities.commands) })}</div>
              <div>{t("settings.cap_config", { value: formatCapability(props.legalworkServerCapabilities.config) })}</div>
              <div>
                {t("settings.cap_browser_tools", {
                  value: (() => {
                    const browser = props.legalworkServerCapabilities.toolProviders?.browser;
              ...
              <div>
                {t("settings.cap_file_tools", {
                  value: (() => {
                    const files = props.legalworkServerCapabilities.toolProviders?.files;
              ...
              <div>
                {t("settings.cap_sandbox", {
                  value: props.legalworkServerCapabilities.sandbox
                    ? `${props.legalworkServerCapabilities.sandbox.backend} (${props.legalworkServerCapabilities.sandbox.enabled ? t("settings.on") : t("settings.off")})`
      ...
      <div className={cardClass}>
        ...
        <div className={subCardClass}>
          <div className="flex items-center justify-between gap-3">
            ...
            <div className={`rounded-full border px-2 py-1 text-[11px] font-medium ${props.legalworkAuditStatus.className}`}>
              {props.legalworkAuditStatus.label}
            </div>
          </div>
          {props.legalworkAuditError ? <StatusBanner tone="error" message={props.legalworkAuditError} /> : null}
          {props.legalworkAuditEntries.length > 0 ? (
            <div className="divide-y divide-dls-border/60">
              {props.legalworkAuditEntries.map((entry) => (
      ...
      {isDesktop ? (
        <div className="space-y-3 rounded-2xl border border-red-7/30 bg-red-3/10 p-5">
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="text-sm font-semibold tracking-[-0.1px] text-dls-text">
                {t("settings.reset_legalwork_title")}
              </div>
              <div className="text-[12px] text-dls-secondary">
                {props.opencodeDevModeEnabled
                  ? t("settings.reset_legalwork_desc_dev")
                  : t("settings.reset_legalwork_desc_prod")}
              </div>
            </div>
            <div
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              onClick={() => void props.onNukeLegalworkAndOpencodeConfig()}
