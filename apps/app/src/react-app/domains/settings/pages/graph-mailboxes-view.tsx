...
export function GraphMailboxesView({client,accounts,onChanged}:{client:MailClient;accounts:MailAccountView[];onChanged:()=>void}) {
 ...
 return <MailSettingsSection title="Microsoft shared and delegated mailboxes">
  <p className="text-xs text-muted-foreground">Connect a mailbox your organization has shared with you.</p>
  {shared.map(account=>{const identity=account.identity!;return <div key={account.id} className="rounded-md border p-3 text-sm space-y-2">
   ...
   <Button variant="outline" size="sm" disabled={busy} onClick={()=>{setAdding(true);setParent(identity.credentialAccountId);setAddress(identity.address);setKind(identity.kind);setWrite(identity.writeConfirmed);setSend(identity.sendMode);setMessage('Review the administrator grants, then verify mailbox access.');}}>Review access</Button>
  </div>;})}
  {!!parents.length&&!adding&&<Button size="sm" variant="outline" onClick={()=>setAdding(true)}>Add shared mailbox</Button>}{!!parents.length&&adding&&<form className="space-y-3" onSubmit={async event=>{event.preventDefault();setBusy(true);setError('');setMessage('');try{await client.configureGraphMailbox({credentialAccountId:parent,address,kind,writeConfirmed:write,sendMode:send},new AbortController().signal);setMessage('Mailbox access verified. Synchronization starts independently of your other mailboxes.');onChanged();}catch{setError('Mailbox access could not be verified. Check the address, Exchange mailbox access, and Microsoft shared-mail permissions. Reconnect the signed-in Microsoft account after administrator consent, then retry.');}finally{setBusy(false);}}}>
   <label className="block text-sm">Signed-in Microsoft account<MailSettingsSelect className="mt-1 block w-full rounded-md border bg-background p-2" aria-label="Signed-in Microsoft account" value={parent} disabled={busy} onChange={value =>setParent(value)}>{parents.map(account=><option key={account.id} value={account.id}>{account.displayName}</option>)}</MailSettingsSelect></label>
   <label className="block text-sm">Mailbox address<Input className="mt-1" type="email" required aria-label="Shared mailbox address" value={address} disabled={busy} onChange={event=>setAddress(event.target.value)} /></label>
   <label className="block text-sm">Mailbox type<MailSettingsSelect className="mt-1 block w-full rounded-md border bg-background p-2" aria-label="Mailbox type" value={kind} disabled={busy} onChange={value =>setKind(value==='delegated'?'delegated':'shared')}><option value="shared">Shared mailbox</option><option value="delegated">Delegated mailbox</option></MailSettingsSelect></label>
   ...
   <label className="block text-sm">Administrator-confirmed sender permission<MailSettingsSelect className="mt-1 block w-full rounded-md border bg-background p-2" aria-label="Administrator-confirmed sender permission" value={send} disabled={busy} onChange={value =>setSend(value==='send_as'?'send_as':value==='send_on_behalf'?'send_on_behalf':'none')}><option value="none">None / unknown</option><option value="send_as">Send as</option><option value="send_on_behalf">Send on behalf</option></MailSettingsSelect></label>
   ...
   <Button type="button" size="sm" variant="ghost" disabled={busy} onClick={()=>setAdding(false)}>Cancel</Button><Button type="submit" variant="outline" disabled={busy||!parent}>{busy?'Verifying access…':'Verify mailbox access'}</Button>
  </form>}
