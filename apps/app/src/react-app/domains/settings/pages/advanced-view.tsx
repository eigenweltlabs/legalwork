...
export type AdvancedViewProps = {
  ...
  getRuntimeConfigStatus: () => Promise<LegalworkRuntimeConfigStatus>;
};
...
export function AdvancedView(props: AdvancedViewProps) {
  ...
  const [configStatus, setConfigStatus] = useState<LegalworkRuntimeConfigStatus | null>(null);
  ...
  const legalworkTone: AdvancedStatusTone = (() => {
    switch (props.legalworkServerStatus) {
      case "connected":
        return "ready";
      case "limited":
  ...
  const clientDetailLines = props.clientConnected
    ...
    : [
        ...
        "LegalWork server config sources below can still be inspected.",
      ];

  const legalworkDetailLines = props.legalworkServerStatus === "connected"
    ? ["Runtime DB, workspace config, and migration diagnostics are available."]
    : ["Runtime config diagnostics need the LegalWork server connection."];
  ...
  return (
    <LayoutStack>
      <AdvancedRuntimeSection
        ...
        legalworkTone={legalworkTone}
        legalworkDetailLines={legalworkDetailLines}
      />
