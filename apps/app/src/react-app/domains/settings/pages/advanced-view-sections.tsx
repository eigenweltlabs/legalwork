...
import type { LegalworkRuntimeConfigStatus, LegalworkServerStatus } from "@/app/lib/legalwork-server";
...
interface AdvancedRuntimeSectionProps {
  ...
  legalworkStatusLabel: string;
  legalworkTone: SettingsTone;
  legalworkDetailLines: string[];
}
    ...
    <LayoutSection>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <RuntimeStatusCard
          ...
          description={t("settings.legalwork_server_desc")}
          statusLabel={props.legalworkStatusLabel}
          tone={props.legalworkTone}
          detailLines={props.legalworkDetailLines}
        />
      </div>
    </LayoutSection>
  );
...
interface AdvancedRuntimeMigrationSectionProps {
  ...
  configStatus: LegalworkRuntimeConfigStatus | null;
      ...
      <LayoutSectionItem className="gap-3">
        ...
        <div className="flex flex-wrap gap-2 pt-2">
          <Button
            ...
            disabled={props.busy || props.legalworkReconnectBusy || !props.legalworkServerUrl.trim()}
          >
            <RefreshCcw size={14} className={props.legalworkReconnectBusy ? "animate-spin" : ""} />
            {props.legalworkReconnectBusy ? t("settings.reconnecting") : t("settings.reconnect_server")}
          </Button>
          ...
          {props.isLocalEngineRunning ? (
            ...
            >
              ...
              {props.restartBusy ? t("settings.restarting") : t("settings.restart_legalwork_server")}
            </Button>
          ) : null}
          ...
          {!props.isLocalEngineRunning && props.legalworkServerStatus === "connected" ? (
            <Button
              type="button"
              variant="outline"
              size="sm"
