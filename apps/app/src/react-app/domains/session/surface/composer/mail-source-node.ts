...
export class ComposerMailSourceNode extends TextNode {
 ...
 override createDOM(_config:EditorConfig){const dom=document.createElement('span');dom.className='inline-flex items-center rounded-full border border-gray-6 bg-gray-3 px-2.5 py-1 text-xs font-medium text-gray-11';dom.contentEditable='false';dom.setAttribute('spellcheck','false');const button=document.createElement('button');button.type='button';button.textContent='Source email';button.title='Return to the source email';button.setAttribute('aria-label','Return to source email');const href=mailSourceLink(this.__text);button.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();event.stopPropagation();if(href)openMailSource(href);}});button.addEventListener('click',()=>{if(href)openMailSource(href);});dom.append(button);return dom;}
