  ...
  return <>
    ...
    <Popover><PopoverTrigger render={<Button variant="outline" size="sm" className="h-8" />}><ListFilter className="size-3.5" />{t("review.filters")}{active && <span className="size-1.5 rounded-full bg-foreground" />}</PopoverTrigger>
      <PopoverContent align="start" className="w-80 gap-4 p-4"><PopoverTitle>{t("review.filters")}</PopoverTitle>
        ...
        {typed && <Field className="gap-1.5"><FieldLabel>{t("review.filter_value")}</FieldLabel><ReviewSelect value={value.operator} label={t("review.filter_comparison")} onChange={operator => update({ operator })} options={[{ value: "any", label: t("review.compare_any") }, { value: "eq", label: t("review.compare_eq") }, { value: "gt", label: t("review.compare_gt") }, { value: "gte", label: t("review.compare_gte") }, { value: "lt", label: t("review.compare_lt") }, { value: "lte", label: t("review.compare_lte") }]} />{value.operator !== "any" && <Input type={column.kind === "date" ? "date" : "number"} step="any" value={value.value} aria-label={t("review.filter_value")} onChange={event => update({ value: event.target.value })} />}</Field>}
