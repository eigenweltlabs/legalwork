...
export function ReviewPromptFields({ column, onChange, kinds = ReviewColumnKindSchema.options, nameLabel = t("review.prompt_name"), disabled }: {
  ...
  return <fieldset disabled={disabled} className="min-w-0 space-y-5">
    <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_180px]">
      ...
      <Field className="gap-1.5"><FieldLabel>{t("review.kind")}</FieldLabel><ReviewSelect disabled={disabled} label={t("review.kind")} value={column.kind} options={kinds.map(value => ({ value, label: t(`review.${value}`) }))} onChange={value => onChange({ kind: ReviewColumnKindSchema.parse(value) })} /></Field>
    </div>
