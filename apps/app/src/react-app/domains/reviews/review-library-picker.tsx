      ...
      <div className="flex min-h-0 flex-1 flex-col sm:flex-row">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-6" aria-label={t("review.inspect_prompt")}>
          {choice && <><h3 className="text-base font-semibold">{choice.title}</h3>{view === "sets" && <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{choice.description}</p>}
            <div className="mt-5 divide-y">{choice.columns.map((column, index) => <div key={column.key} className="py-4 first:pt-0">{view === "sets" && <h4 className="mb-2 text-sm font-medium"><span className="mr-2 text-muted-foreground">{index + 1}.</span>{column.label}</h4>}<p className="whitespace-pre-wrap text-sm leading-relaxed">{column.question}</p><p className="mt-2 text-xs text-muted-foreground">{t(`review.${column.kind}`)}</p>{!!column.options.length && <ul className="mt-2 list-inside list-disc space-y-1 text-xs text-muted-foreground">{column.options.map(option => <li key={option}>{option}</li>)}</ul>}{column.hint && <details className="mt-3 text-xs text-muted-foreground"><summary className="cursor-pointer">{t("review.instructions")}</summary><p className="mt-2 whitespace-pre-wrap leading-relaxed">{column.hint}</p></details>}</div>)}</div></>}
        </div>
      </div>
