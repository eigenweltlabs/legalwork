...
export class MailOnboardingClient {
  ...
  begin(choice: AccountChoice, signal: AbortSignal, reconnectAccountId?: string, contacts=false) {
    signal.throwIfAborted();
    return this.client.request('/connections', started, AbortSignal.timeout(15000), { provider: choice === 'gmail' ? 'gmail' : 'graph', ...(contacts?{contacts:true}:{}), ...(choice === 'gmail' ? {} : { personal: choice === 'outlook' }), ...(reconnectAccountId ? { reconnectAccountId } : {}) });
  }
