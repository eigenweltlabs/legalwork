...
export class ComposeSaveQueue {
 ...
 constructor(private client:MailClient,readonly account:string,readonly id:string,version:MailDraftView['version']|null,private changed:()=>void){this.version=version;}
 save(content:MailDraftSave['content']){this.pending=structuredClone(content);this.error='';if(!this.running){this.running=this.flush().finally(()=>{this.running=undefined;this.changed();if(this.pending&&!this.error)void this.save(this.pending);});}this.changed();return this.running;}
 private async flush(){while(this.pending){const content=this.pending;this.pending=undefined;try{const saved=await this.client.request(`/accounts/${encodeURIComponent(this.account)}/drafts/save`,mailDraftViewSchema,new AbortController().signal,{draftId:this.id,expected:this.version,content});this.version=saved.version;}catch(error){this.pending=this.pending??content;this.error=error instanceof Error?(error.message.includes('(409)')?'This draft changed elsewhere. Save a copy to keep your edits.':error.message):'Draft could not be saved.';break;}this.changed();}}
}
