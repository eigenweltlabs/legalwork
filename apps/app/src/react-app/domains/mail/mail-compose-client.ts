...
export class ComposeSaveQueue {
 ...
 save(content:MailDraftSave['content']){this.pending=structuredClone(content);this.error='';this.recovered=false;this.recoveryError='';const journal=this.journal.write(this.pending,this.version);const previous=this.journalWrite;this.journalWrite=journal;if(previous?.completion!==journal.completion)void journal.completion.then(result=>{if(this.journalWrite?.completion!==journal.completion||this.journalWrite.sequence!==result.sequence)return;this.recovered=!!result.token;this.changed();},()=>{if(this.journalWrite?.completion!==journal.completion)return;this.recoveryError='Device recovery could not be saved. Keep this draft open until local saving succeeds.';this.changed();});if(!this.running){this.running=this.flush().finally(()=>{this.running=undefined;this.changed();if(this.pending&&!this.error)void this.save(this.pending);});}this.changed();return this.running;}
 private async flush(){while(this.pending){const content=this.pending,journal=this.journalWrite;this.pending=undefined;try{const result=await journal?.completion.catch(()=>null),token=result?.sequence===journal?.sequence?result?.token:null;const saved=await this.client.request(`/accounts/${encodeURIComponent(this.account)}/drafts/save`,mailDraftViewSchema,new AbortController().signal,{draftId:this.id,expected:this.version,content});this.version=saved.version;await this.journal.acknowledge(token??null).catch(()=>{});this.recoveryError='';}catch(error){this.pending=this.pending??content;this.error=error instanceof Error?(error.message.includes('(409)')?'This draft changed elsewhere. Save a copy to keep your edits.':error.message):'Draft could not be saved.';break;}this.changed();}}
}
