...
export function mailDocument(html:string,inline:ReadonlyMap<string,string>=new Map(),remote:ReadonlyMap<string,string>=new Map(),nonce?:string,imageBudget=16*1024*1024):MailDocument {
 ...
 if(nonce){const script=doc.createElement('script');script.setAttribute('nonce',nonce);script.setAttribute('style','display:none!important');script.textContent=`(()=>{const token=${JSON.stringify(nonce)};let queued=false,last=0;const size=()=>{queued=false;const b=document.body,s=getComputedStyle(b),height=Math.min(20000,Math.max(80,Math.ceil(Math.max(b.scrollHeight,b.getBoundingClientRect().height)+(parseFloat(s.marginTop)||0)+(parseFloat(s.marginBottom)||0)+32+(document.documentElement.scrollWidth>innerWidth?20:0))));if(height!==last){last=height;parent.postMessage({type:'mail-size',token,height},'*');}};const schedule=()=>{if(!queued){queued=true;requestAnimationFrame(size);}};addEventListener('message',event=>{if(event.source===parent&&event.data?.type==='mail-measure'&&event.data.token===token){last=0;schedule();}});new ResizeObserver(schedule).observe(document.body);addEventListener('load',schedule);addEventListener('resize',schedule);document.addEventListener('click',e=>{const a=e.target.closest?.('[data-mail-link]');if(a){e.preventDefault();if(e.isTrusted)parent.postMessage({type:'mail-link',token,index:Number(a.getAttribute('data-mail-link'))},'*');}});document.addEventListener('keydown',e=>{if(e.isTrusted&&!e.defaultPrevented&&!e.repeat&&!e.isComposing&&!e.getModifierState('AltGraph')&&!getSelection()?.toString()&&!e.target.closest?.('input,textarea,select,[contenteditable]')&&(${mailReaderKeyAllowed.toString()})(e,${/Mac/i.test(navigator.platform)})){e.preventDefault();parent.postMessage({type:'mail-reader-key',token,key:e.key,ctrlKey:e.ctrlKey,metaKey:e.metaKey,altKey:e.altKey,shiftKey:e.shiftKey},'*');return;}if(e.key==='Enter'&&e.target.matches?.('[data-mail-link]')){e.preventDefault();if(e.isTrusted)parent.postMessage({type:'mail-link',token,index:Number(e.target.getAttribute('data-mail-link'))},'*');}});schedule();})();`;bridge=script.outerHTML.replace('nonce=""','nonce="'+nonce+'"');}
