...
export function MailRoute({chatBridge}:{chatBridge?:MailChatBridge}) {
    ...
    const openCompose=(value:ComposeSelection)=>{openGeneration.current++;setReaderAction(undefined);if(!compose&&document.activeElement instanceof HTMLElement)composeReturn.current=document.activeElement;setCompose(value);setDraftsOpen(false);setOutboxOpen(false);};
...
<button className="mail-nav-row" title="Outbox" aria-label="Open outbox" disabled={locked||!!compose} onClick={()=>{setDraftsOpen(false);setOutboxOpen(true);}}><SendHorizontal size={16}/><span>Outbox</span></button><button className="mail-nav-row" title="Drafts" aria-label="Open drafts" disabled={locked||!!compose} onClick={()=>{setOutboxOpen(false);setDraftsOpen(true);}}><FileText size={16}/><span>Drafts</span></button><button className="mail-nav-row" aria-label="Save mail to connected storage" disabled={!client||locked} onClick={()=>{setMatterFiling(false);setFilingOpen(true);}}><FolderPlus size={15}/><span>Save to storage</span></button>
