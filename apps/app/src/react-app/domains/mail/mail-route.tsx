import {MailConversation,conversationId} from './mail-conversation';
...
import {MailActionsClient,type ActionEntry} from './mail-actions-client';
...
import { MailClient, MailRequestError, UnifiedMailPages, bodyEnvelope, type MailAccountView, type MailFolderView, type MailMessageView, type MailPartView, type SyncStatus } from './mail-client';
...
const textError = (error: unknown) => error instanceof Error ? error.name==='ZodError'?'Mail returned an incomplete response. Refresh mail to retry.':error.message : 'Mail is unavailable.';
...
export function MailRoute({chatBridge}:{chatBridge?:MailChatBridge}) {
    ...
    const composeOrigin=useRef<'drafts'|'outbox'|null>(null);
    const [draftsOpen,setDraftsOpen]=useState(false);const [outboxOpen,setOutboxOpen]=useState(false);
    const [listWidth,setListWidth]=useState(()=>{try{return Math.max(320,Math.min(520,Number(localStorage.getItem('mail.layout.listWidth'))||380));}catch{return 380;}});
    const resizeList=(width:number)=>{const next=Math.max(320,Math.min(520,width));setListWidth(next);try{localStorage.setItem('mail.layout.listWidth',String(next));}catch{/* Layout preferences are optional. */}};
    ...
    const showWindow=(rows:MailMessageView[])=>{setItems(rows);const retained=new Set(listWindow.current.retained.map(mailItemId));setChecked(current=>new Set([...current].filter(id=>retained.has(id))));const top=listWindow.current.scroll,epoch=messageRequest.current;requestAnimationFrame(()=>{if(epoch===messageRequest.current&&listScroll.current)listScroll.current.scrollTop=top;});};
    ...
    const [readerAction,setReaderAction]=useState<{generation:number;item:MailMessageView;action:MailReaderAction;client:MailClient|undefined}>();
    const openMessage=(item:MailMessageView|undefined,action?:MailReaderAction)=>{const generation=++openGeneration.current;setReaderAction(item&&action?{generation,item,action,client}:undefined);if(!item){setSelected(undefined);return;}setSelected(item);void markOpened(item);setCompose(undefined);setFilingOpen(false);setDraftsOpen(false);setOutboxOpen(false);};
    ...
    useEffect(()=>{
      ...
      const controller=new AbortController();void(async()=>{
        ...
        if(controller.signal.aborted)return;notificationOpened.current=raw;openMessage(message);setError('');setCompose(undefined);setDraftsOpen(false);setOutboxOpen(false);
    ...
    const purge = () => { openGeneration.current++;setReaderAction(undefined); setEnqueued([]);messageRequest.current++; request.current.abort(); request.current = new AbortController(); pager.current = undefined; listWindow.current.reset();setNewerAvailable(false);setItems([]); setSelected(undefined); setSync(undefined); setMore(false); };
    useEffect(() => {
    ...
    const waitingAccounts = accounts.filter(value => (!account || value.id === account) && accountSync[value.id]?.state === 'waiting');
    const downloading = accounts.some(value => (!account || value.id === account) && accountSync[value.id]?.state === 'syncing');
    const refreshLabel = waitingAccounts.length ? 'Waiting to retry · Retry sync' : downloading ? 'Downloading mail · Refresh mail' : 'Refresh mail';
    ...
    const currentTargets=useRef(actionTargets);currentTargets.current=actionTargets;
    const matchesReaderTarget=(item:MailMessageView)=>currentTargets.current.length===1&&currentTargets.current[0].accountId===item.accountId&&JSON.stringify(currentTargets.current[0].locator)===JSON.stringify(item.locator);
    useEffect(()=>{
      if(compose||filingOpen||draftsOpen||outboxOpen){openGeneration.current++;setReaderAction(undefined);return;}
      if(!readerAction)return;
      const root=mailRoot.current,identity=JSON.stringify([readerAction.item.accountId,readerAction.item.locator]);
      if(!root||locked||client!==readerAction.client||!selected||JSON.stringify([selected.accountId,selected.locator])!==identity||!matchesReaderTarget(readerAction.item)){setReaderAction(undefined);openGeneration.current++;return;}
      const attempt=()=>{
        if(openGeneration.current!==readerAction.generation||!matchesReaderTarget(readerAction.item))return;
        const article=root.querySelector<HTMLElement>('#mail-print-root');
        if(article?.dataset.mailIdentity!==identity)return;
        const control=article.querySelector<HTMLButtonElement>(`[aria-label="${readerAction.action==='reply-all'?'Reply all':readerAction.action==='reply'?'Reply':'Forward'}"]`);
        if(control&&!control.disabled){setReaderAction(undefined);control.click();}
      };
      const observer=new MutationObserver(attempt);observer.observe(root,{subtree:true,childList:true,attributes:true,attributeFilter:['disabled','data-mail-identity']});attempt();return()=>observer.disconnect();
    },[readerAction,selected,client,locked,checked,searchTargets,compose,filingOpen,draftsOpen,outboxOpen]);
    const renderGeneration=openGeneration.current;
    const closeCompose=()=>{setCompose(undefined);if(composeOrigin.current==='drafts')setDraftsOpen(true);if(composeOrigin.current==='outbox')setOutboxOpen(true);requestAnimationFrame(()=>{const target=composeReturn.current;if(target?.isConnected)target.focus();else mailRoot.current?.querySelector<HTMLButtonElement>('.mail-message-row[aria-pressed="true"],.mail-message-row')?.focus();});};
    const openCompose=(value:ComposeSelection)=>{if(!compose)composeOrigin.current=draftsOpen?'drafts':outboxOpen?'outbox':null;openGeneration.current++;setReaderAction(undefined);if(!compose&&document.activeElement instanceof HTMLElement)composeReturn.current=document.activeElement;setCompose(value);setDraftsOpen(false);setOutboxOpen(false);};
    const workView=draftsOpen||outboxOpen||!!compose&&composeOrigin.current!==null;
    const reader = filingOpen&&client ? (matterFiling?<MailFilingPanel client={client} accountIds={account?[account]:accounts.map(value=>value.id)} message={actionTargets.length===1?actionTargets[0]:selected} onClose={()=>setFilingOpen(false)}/>:<MailStorageSavePanel client={client} accountIds={account?[account]:accounts.map(value=>value.id)} message={actionTargets.length===1?actionTargets[0]:selected} onClose={()=>setFilingOpen(false)} onMatter={()=>setMatterFiling(true)}/>) : compose&&client ? <MailComposer returnTo={composeOrigin.current??undefined} key={compose.account+compose.id} client={client} accounts={accounts.filter(value=>value.provider!=='archive')} initial={compose} onClose={closeCompose} onSwitch={openCompose} onQueued={()=>{setCompose(undefined);setOutboxOpen(true);}}/> : outboxOpen&&client ? <MailOutbox onOpen={openCompose} client={client} accounts={accounts.filter(value=>value.provider!=='archive')} onClose={()=>setOutboxOpen(false)}/> : draftsOpen&&client ? <MailDrafts client={client} accounts={accounts.filter(value=>value.provider!=='archive')} onOpen={openCompose} onClose={()=>setDraftsOpen(false)}/> : selected && client ? <MailConversation grouped={!searching} key={conversationId(selected)+String(searching)} client={client} item={selected} onOpen={openMessage} onInitialOpen={markOpened} entries={actionEntries} render={(message,active,onRemoved)=><MailReader onRemoved={onRemoved} client={client} item={message} active={active} chatBridge={chatBridge}
      onCompose={value=>{if(openGeneration.current===renderGeneration&&(!readerAction||matchesReaderTarget(readerAction.item)))openCompose(value);}} account={accounts.find(value=>value.id===message.accountId)?.displayName??message.accountId}
    ...
    return (
      <main ref={mailRoot} tabIndex={-1} className="mail-workspace" data-mail-selecting={selecting||checked.size>0||searchTargets.length>1} data-mail-selection={mailSelectionSignature(actionTargets)} data-mail-reader={JSON.stringify([selected?.accountId,selected?.key,compose?.id,filingOpen,draftsOpen,outboxOpen])} aria-label="Local mail" onKeyDown={event=>mailKeyboard(event.nativeEvent,event.currentTarget,{showFolders:()=>setFoldersOpen(true)})}>
        <header className="mail-toolbar">
          ...
          <h1 className="sr-only">Mail</h1><button className="mail-compose-launch" {...mailShortcutProps('compose','Compose')} aria-label="Compose" disabled={locked||!accounts.some(value=>value.provider!=='archive')||!!compose} onClick={()=>{const selectedAccount=accounts.find(value=>value.id===account&&value.provider!=='archive')??accounts.find(value=>value.provider!=='archive');if(!selectedAccount)return;openCompose({account:selectedAccount.id,id:crypto.randomUUID(),version:null,content:emptyCompose(selectedAccount.identity?.address??addresses(selectedAccount.displayName)[0]??'')});}}><SquarePen size={16}/><span>New mail</span></button>{!draftsOpen&&!outboxOpen&&!compose&&!locked&&client&&!actionTargets.some(item=>item.locator.provider==='archive')&&<MailActionBar enqueued={enqueued} reading={!searching&&checked.size===0&&!!selected} blocked={searching&&searchPending} client={client} accounts={accounts} accountId={account} selected={actionTargets} folder={folders.find(item=>item.id===folder)} folders={folders} onEntries={setActionEntries} onRefresh={()=>setActionPulse(value=>value+1)}/>}

          <div className="mail-toolbar-actions">



            {!draftsOpen&&!outboxOpen&&<label className="mail-toolbar-search"><Search size={15}/><input {...mailShortcutProps('search','Search mail')} aria-label="Search mail" placeholder={savedSearchName ? `Saved: ${savedSearchName}` : 'Search'} disabled={locked} value={searchQuery} onChange={event => { setSearchQuery(event.target.value); setSavedSearchName(''); }}/>{searching && <button aria-label="Clear search" onClick={() => setSearching(false)}><X size={13}/></button>}</label>}
            <button className="mail-icon-button" title={refreshLabel} aria-label={refreshLabel} onClick={() => waitingAccounts.length ? void Promise.all(waitingAccounts.map(value => control('start', value.id))) : refresh()}><RefreshCw size={16}/></button>
            <MailKeyboardHelp/><button className="mail-icon-button" title="Mail accounts" aria-label="Mail accounts" onClick={openSettings}><Settings2 size={17}/></button>
          </div>
        </header>
        ...
        {locked ? <div className="mail-empty"><div className="mail-empty-icon"><Mail size={24}/></div><h2 role="status">{error ? 'Mail is unavailable' : 'Opening your mail…'}</h2>{error && <Button variant="outline" size="sm" onClick={refresh}>Try again</Button>}</div>
          : <div className={`mail-grid ${workView?'mail-work-layout':''} ${selected||compose||draftsOpen||outboxOpen ? 'has-selection' : ''} ${foldersOpen ? 'folders-open' : ''}`}>
            <nav tabIndex={-1} aria-label="Mail accounts and folders" className="mail-folders">
              <div className="mail-nav-caption">Mailboxes</div>
              <button className={`mail-nav-row ${!workView&&!account && inbox ? 'is-active' : ''}`} onClick={() => { setDraftsOpen(false);setOutboxOpen(false);setSearching(false); setThread(undefined); setFolder(''); setInbox(true); setAccount(''); setFoldersOpen(false); }}><Inbox size={16}/><span>Inbox</span></button>
              <button className={`mail-nav-row ${!workView&&!account && !inbox ? 'is-active' : ''}`} onClick={() => { setDraftsOpen(false);setOutboxOpen(false);setSearching(false); setThread(undefined); setFolder(''); setInbox(false); setAccount(''); setFoldersOpen(false); }}><Archive size={16}/><span>All mail</span></button>
<button className={`mail-nav-row ${outboxOpen||compose&&composeOrigin.current==='outbox'?'is-active':''}`} title="Outbox" aria-label="Open outbox" disabled={locked||!!compose} onClick={()=>{setDraftsOpen(false);setOutboxOpen(true);}}><SendHorizontal size={16}/><span>Outbox</span></button><button className={`mail-nav-row ${draftsOpen||compose&&composeOrigin.current==='drafts'?'is-active':''}`} title="Drafts" aria-label="Open drafts" disabled={locked||!!compose} onClick={()=>{setOutboxOpen(false);setDraftsOpen(true);}}><FileText size={16}/><span>Drafts</span></button><button className="mail-nav-row" aria-label="Save mail to connected storage" disabled={!client||locked} onClick={()=>{setMatterFiling(false);setFilingOpen(true);}}><FolderPlus size={15}/><span>Save to storage</span></button>
              <div className="mail-nav-caption mail-account-caption">Accounts</div>
              {accounts.map(value => <div key={value.id}><button className={`mail-nav-row mail-account-row ${!workView&&account === value.id ? 'is-active' : ''}`} onClick={() => { setDraftsOpen(false);setOutboxOpen(false);setSearching(false); setThread(undefined); setAccount(value.id); if(value.provider==='archive')setInbox(false); }}><span className="mail-account-dot">{value.displayName.slice(0, 1).toUpperCase()}</span><span><span className="mail-account-name">{senderName(value.displayName)}</span><small title={value.displayName}>{accountLabel(value.provider)}</small></span></button>{account === value.id && <div className="mail-account-folders">{folders.map(folderEntry => <button key={folderEntry.id} className={`mail-nav-row ${folder === folderEntry.id ? 'is-active' : ''}`} title={folderEntry.name} onClick={() => { setDraftsOpen(false);setOutboxOpen(false);setSearching(false); setThread(undefined); setFolder(folderEntry.id); setFoldersOpen(false); }}>{folderEntry.role === 'inbox' ? <Inbox size={15}/> : <Folder size={15}/>}<span>{folderEntry.name}</span></button>)}</div>}</div>)}
            ...
            {!workView&&<section tabIndex={-1} aria-label="Messages" className="mail-message-list" style={{width:`clamp(320px, max(${listWidth}px, 27cqw), 560px)`}}><div className="mail-pane-resize" role="separator" aria-label="Message list width" aria-orientation="vertical" aria-valuemin={320} aria-valuemax={520} aria-valuenow={listWidth} aria-valuetext={`${listWidth} pixels, limited by available space`} title="Drag or use Arrow keys to resize. Double-click to reset." tabIndex={0} onDoubleClick={()=>resizeList(380)} onKeyDown={event=>{if(event.key==='ArrowLeft'||event.key==='ArrowRight'){event.preventDefault();event.stopPropagation();resizeList(listWidth+(event.key==='ArrowRight'?20:-20));}}} onPointerDown={event=>{event.currentTarget.setPointerCapture(event.pointerId);event.currentTarget.dataset.origin=String(event.clientX-listWidth);}} onPointerMove={event=>{if(event.currentTarget.hasPointerCapture(event.pointerId))resizeList(event.clientX-Number(event.currentTarget.dataset.origin));}} onPointerUp={event=>{if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId);}}/>
              {searching && client ? <MailSearch client={client} accounts={accounts} toolbarQuery={searchQuery.trim()} onSavedQuery={(text, name) => { setSearchQuery(text); setSavedSearchName(name); }} onOpen={openMessage} onTargets={updateSearchTargets}/> : <>
              ...
              {accounts.filter(value=>!account||value.id===account).map(value=>{const progress=accountSync[value.id];if(!progress||value.provider==='archive'||!['attention','paused'].includes(progress.state))return null;return <div key={value.id} className="mail-sync-progress" role="status"><CircleAlert size={13}/><span>{senderName(value.displayName)} · {progress.state==='paused'?'Paused':'Connection needs attention'}</span><button onClick={()=>progress.state==='paused'?control('start',value.id):openSettings()}>{progress.state==='paused'?'Resume':'Review account'}</button></div>;})}
              ...
              <div ref={listScroll} className="mail-message-scroll" aria-label="Message navigation: Up and Down browse, Enter opens">
                {items.map(observed => {const value=optimisticMail(observed,actionEntries);return <MailRowMenu key={mailItemId(value)} prepare={()=>{const targets=checked.has(mailItemId(value))?items.filter(item=>checked.has(mailItemId(item))):[value];if(!checked.has(mailItemId(value)))setChecked(new Set([mailItemId(value)]));return{signature:mailSelectionSignature(targets),count:targets.length,archive:targets.some(item=>item.locator.provider==='archive'),mixed:targets.some(item=>item.accountId!==targets[0].accountId)};}} onOpen={action=>openMessage(value,action)}><div className="mail-message-select-row"><input type="checkbox" aria-label={`Select ${value.subject||'message'}`} checked={checked.has(mailItemId(value))} onChange={event=>{const enabled=event.target.checked;setChecked(current=>{const next=new Set(current);if(enabled)next.add(mailItemId(value));else next.delete(mailItemId(value));return next;});}}/><button className={`mail-message-row ${!!selected && conversationId(selected) === conversationId(value) ? 'is-selected' : ''} ${(value.conversation ? value.conversation.unreadCount>0 : value.isRead===false) ? 'is-unread' : ''}`} tabIndex={items.some(item=>selected&&conversationId(item)===conversationId(selected))?selected&&conversationId(value)===conversationId(selected)?0:-1:items[0]?.accountId===value.accountId&&items[0]?.key===value.key?0:-1} onClick={event => {const id=mailItemId(value),anchor=selectionAnchor.current;if(event.metaKey||event.ctrlKey||event.shiftKey){setChecked(current=>{const next=new Set(current);if(event.shiftKey&&anchor){const from=items.findIndex(item=>mailItemId(item)===anchor),to=items.findIndex(item=>mailItemId(item)===id);if(from>=0)items.slice(Math.min(from,to),Math.max(from,to)+1).forEach(item=>next.add(mailItemId(item)));else next.add(id);}else if(next.has(id))next.delete(id);else next.add(id);return next;});}else openMessage(value);selectionAnchor.current=id;}} aria-pressed={!!selected && conversationId(selected) === conversationId(value)}>
                  ...
                  <span className="mail-row-bottom">{value.preview&&<span className="mail-row-preview">{value.preview}</span>}{!account&&<span className="mail-row-account" title={accounts.find(entry=>entry.id===value.accountId)?.displayName}>{senderName(accounts.find(entry=>entry.id===value.accountId)?.displayName)}</span>}{value.contentState !== 'complete' && <span title={value.contentState === 'downloading' ? 'Content is downloading' : 'Content is unavailable'}><CircleAlert size={12}/></span>}</span>
                ...
                {!busy&&!items.length&&error&&<div className="mail-list-empty"><CircleAlert size={24}/><p>Messages could not be loaded</p><Button size="sm" variant="outline" onClick={refresh}>Retry loading messages</Button></div>}{busy && <p role="status" className="mail-loading">Loading messages…</p>}
              </div>
            ...
            <section tabIndex={-1} aria-label="Message reader" className="mail-reader-pane">{selected&&!workView && <button className="mail-back" onClick={() => setSelected(undefined)}><ChevronLeft size={15}/>Inbox</button>}{reader}</section>
          </div>}
      </main>
    );
}
...
function MailReader({ active=true, chatBridge, client, item:observedItem, account, onThread, onUnavailable, onRemoved, onCompose }: {
    ...
    const item:MailMessageView={...observedItem,rawReferenceId:contentItem.rawReferenceId,contentState:contentItem.contentState};
    useEffect(()=>{setContentItem(observedItem);},[observedItem.rawReferenceId,observedItem.contentState]);
    ...
    useEffect(() => { const abort = new AbortController(); controller.current = abort;loadingContent.current=true;reloadAfterValidation.current=false; void (async () => { try {
        ...
        if (abort.signal.aborted)
            return;
        let nextBodies: {contentType:string;text:string;presentation?:boolean}[]=[];
    ...
    async function composeMessage(mode:ComposeMode){const signal=controller.current.signal;setBusy(true);try{
      ...
      if(!signal.aborted)onCompose({account:item.accountId,id:crypto.randomUUID(),version:null,content});
    }catch(error){setError(textError(error));}finally{setBusy(false);}}
    return (
      <article id={active?"mail-print-root":undefined} className="mail-message" data-mail-identity={JSON.stringify([item.accountId,item.locator])} tabIndex={-1}>
        ...
        {active&&<div className="mail-message-actions">
          ...
          {item.threadId && <button title="View conversation" aria-label="View conversation" onClick={onThread}><MessagesSquare size={15}/></button>}
          <div/>
