...
export type MailReaderAction='reply'|'reply-all'|'forward';
...
export function MailRowMenu({children,prepare,onOpen,className}:{children:ReactNode;prepare:()=>MailMenuSelection|Promise<MailMenuSelection>;onOpen:(action?:MailReaderAction)=>void;className?:string}){
 ...
 const reader:Array<[string,MailReaderAction,MailCommand]>=[['Reply','reply','reply'],['Reply all','reply-all','replyAll'],['Forward','forward','forward']];
 ...
 {selection.count===1&&<><ContextMenuItem onClick={()=>{if(prepared.current===signature(root()))onOpen();}}>Open</ContextMenuItem>{reader.map(([label,action,command])=><ContextMenuItem key={label} disabled={selection.archive} onClick={()=>{if(prepared.current===signature(root()))onOpen(action);}}>{label}<span className="mail-menu-shortcut">{mailShortcut(command).hint}</span></ContextMenuItem>)}<ContextMenuSeparator/></>}
