...
export function MailConversation({grouped=true,client,item,entries,onOpen,onInitialOpen,render}:{grouped?:boolean;client:MailClient;item:MailMessageView;entries:ActionEntry[];onOpen:(item:MailMessageView)=>void;onInitialOpen:(item:MailMessageView)=>void;render:(item:MailMessageView,active:boolean)=>ReactNode}){
 ...
 async function load(cursor?:string,expose=true){
  ...
  if(loading.current||controller.current.signal.aborted)return;
  loading.current=true;const signal=controller.current.signal;setBusy(true);
  try{
 ...
 useEffect(()=>{controller.current=new AbortController();void load();const timer=setInterval(()=>void load(undefined,false),5000);return()=>{controller.current.abort();clearInterval(timer);};},[client]);
