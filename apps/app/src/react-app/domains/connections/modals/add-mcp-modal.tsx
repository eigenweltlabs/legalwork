...
export function AddMcpModal(props: AddMcpModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (state.serverType === "remote") {
      ...
      // A static API token connects via Authorization: Bearer and skips OAuth — the way
      // to connect token-authed servers (e.g. iManage) that reject the loopback OAuth.
          ...
          {state.serverType === "remote" ? (
            <div className="space-y-3">
              ...
              <div className="text-[11px] leading-relaxed text-dls-secondary">
                Sent as <span className="font-mono">Authorization: Bearer …</span>. Use this for servers that
                reject the engine&apos;s OAuth (e.g. iManage, GitHub) — it connects directly with your token.
              </div>
