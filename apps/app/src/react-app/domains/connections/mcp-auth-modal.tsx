    ...
    >
      <DialogContent className="flex max-h-[90vh] min-h-0 w-full max-w-lg flex-col overflow-hidden sm:max-w-lg">
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto">
          {isBusy ? (
            <div className="space-y-4 rounded-[20px] border border-dls-border bg-dls-hover px-5 py-6 text-center">
              <div className="flex items-center justify-center">
                <Loader2 size={32} className="animate-spin text-dls-accent" />
              </div>
              <div className="space-y-2">
                <p className="text-sm font-medium text-dls-text">{t("mcp.auth.waiting_authorization")}</p>
                <p className="text-xs text-dls-secondary">{t("mcp.auth.follow_browser_steps")}</p>
                <button
                  type="button"
                  className="text-xs text-dls-secondary underline underline-offset-2 transition-colors hover:text-dls-text"
                  onClick={handleRetry}
                >
          ...
          {!isBusy && isPreparingReload ? (
            <div className="space-y-4 rounded-[20px] border border-amber-6/60 bg-amber-2/40 px-5 py-6 text-center">
              <div className="flex items-center justify-center">
              ...
              {props.reloadBlocked && (props.activeSessions?.length ?? 0) > 0 ? (
                <div className="space-y-2 text-left">
                  {(props.activeSessions ?? []).map((session) => (
                    <div
                      key={session.id}
                      className="flex items-center justify-between gap-3 rounded-xl border border-amber-6/50 bg-amber-1/40 px-3 py-2"
                    >
                      <span className="text-xs text-dls-text">
          ...
          {!isBusy && alreadyConnected ? (
            <div className="space-y-4 rounded-[20px] border border-emerald-7/20 bg-emerald-3/20 p-5">
              <div className="flex items-center gap-3">
                <div className="flex size-10 shrink-0 items-center justify-center rounded-full bg-emerald-3">
                  <CheckCircle2 size={24} className="text-emerald-11" />
                </div>
                <div>
                  <p className="text-sm font-medium text-dls-text">{t("mcp.auth.already_connected")}</p>
                  <p className="text-xs text-dls-secondary">
                    {t("mcp.auth.already_connected_description", { server: serverName })}
                  </p>
                </div>
              </div>
              <p className="text-xs text-dls-secondary">{t("mcp.auth.configured_previously")}</p>
            </div>
          ) : null}

          {reloadNotice ? (
            <div className="space-y-3 rounded-[20px] border border-dls-border bg-dls-hover p-4">
              <p className="text-sm text-dls-text">{reloadNotice}</p>
          ...
          {!isBusy && authorizationUrl && props.isRemoteWorkspace && !alreadyConnected ? (
            <div className="space-y-3 rounded-[20px] border border-dls-border bg-dls-hover p-4">
              ...
              <div className="flex items-center gap-3 rounded-xl border border-dls-border bg-dls-surface px-3 py-2">
                <div className="min-w-0 flex-1">
                  <div className="text-[10px] uppercase tracking-wide text-dls-secondary">
                    {t("mcp.auth.authorization_link")}
                  </div>
                  <div className="truncate font-mono text-[11px] text-dls-text">{authorizationUrl}</div>
                </div>
              ...
              />
              <div className="text-[11px] text-dls-secondary">{t("mcp.auth.port_forward_hint")}</div>
            ...
            <>
              <div className="space-y-4">
                <div className="flex items-start gap-3">
                  <div className="flex size-6 shrink-0 items-center justify-center rounded-full bg-dls-hover text-xs font-medium text-dls-secondary">
                    1
                  </div>
                  <div>
                    <p className="text-sm font-medium text-dls-text">{t("mcp.auth.step1_title")}</p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-dls-text">{t("mcp.auth.step2_title")}</p>
                    <p className="mt-1 text-xs text-dls-secondary">{t("mcp.auth.step2_description")}</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-dls-text">{t("mcp.auth.step3_title")}</p>
                    <p className="mt-1 text-xs text-dls-secondary">{t("mcp.auth.step3_description")}</p>
                  </div>
                </div>
              </div>

              <div className="rounded-[20px] border border-dls-border bg-dls-hover p-4 text-sm text-dls-text">
                <div className="space-y-3">
                  <p>{t("mcp.auth.waiting_authorization")}</p>
                  <p className="text-xs text-dls-secondary">{t("mcp.auth.follow_browser_steps")}</p>
                  <button
                    type="button"
                    className="text-left text-xs text-dls-secondary underline underline-offset-2 transition-colors hover:text-dls-text"
                    onClick={handleRetry}
                  >
                    {t("mcp.auth.reopen_browser_link")}
                  </button>
