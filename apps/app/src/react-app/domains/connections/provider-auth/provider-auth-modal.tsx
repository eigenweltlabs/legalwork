...
export default function ProviderAuthModal(props: ProviderAuthModalProps) {
  ...
  const entries = useMemo<ProviderAuthEntry[]>(() => {
    ...
    if (props.showLegalWorkModelsSubscribe) {
      const connectedToLegalWork = connected.has(OPENWORK_MODELS_PROVIDER_ID);
      return [
        {
          id: OPENWORK_MODELS_PROVIDER_ID,
          name: "LegalWork",
          methods: [{ type: "cloud", label: "Subscribe" }],
          connected: connectedToLegalWork,
          env: [],
        },
    ...
    return nextEntries;
  }, [isRemoteWorker, props.authMethods, props.connectedProviderIds, props.providers, props.showLegalWorkModelsSubscribe]);
  ...
  const handleEntrySelect = (entry: ProviderAuthEntry) => {
    ...
    if (props.showLegalWorkModelsSubscribe && entry.id.trim().toLowerCase() === OPENWORK_MODELS_PROVIDER_ID) {
            ...
            <div className="-mr-1 min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">
              ...
              {resolvedView === "cloud" && selectedEntry && selectedCloudMethod ? (
                <div className="rounded-xl border border-gray-6/40 bg-gray-2/50 shadow-sm p-5 space-y-4">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="text-[11px] text-gray-9">
                      LegalWork will install the provider config and use the credential stored for your org.
                    </div>
              ...
              {resolvedView === "openwork-subscribe" && selectedEntry ? (
                <div className="rounded-xl border border-blue-6/50 bg-blue-2/25 shadow-sm p-5 space-y-4">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <div className="text-sm font-medium text-gray-12">LegalWork Models</div>
                      <div className="text-xs text-gray-10 mt-1">
                  ...
                  <div className="flex items-center justify-end">
                    <Button onClick={() => void props.onSubscribeLegalWorkModels?.()} disabled={actionDisabled}>
                      Subscribe
                    </Button>
                  </div>
                </div>
