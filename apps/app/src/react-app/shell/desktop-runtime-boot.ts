...
function isLegalworkServerInfoLike(info: unknown): info is BootLegalworkServerInfo {
  return typeof info === "object" && info !== null;
}

function isLegalworkServerReady(info?: BootLegalworkServerInfo) {
  return Boolean(
    info?.running === true &&
      info.baseUrl?.trim() &&
 ...
 *   2) if a local workspace is selected, restart the embedded LegalWork server
 *   3) start the OpenCode engine pointed at the workspace
 *   4) activate the workspace on the running LegalWork server
 ...
 *      localhost ports/tokens into LegalWork settings.
 *
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        hydrateLegalworkServerSettingsFromEnv();
        const preferredRemoteAccess = readLegalworkServerSettings().remoteAccessEnabled === true;

        const publishLegalworkServerInfo = (serverInfo: BootLegalworkServerInfo | null | undefined) => {
        ...
        const startServerWithoutDesktopWorkspace = async () => {
          setPhase("starting-engine", "Starting LegalWork server");
          const serverInfo = await legalworkServerRestart({ remoteAccessEnabled: preferredRemoteAccess }).catch((error) => {
            console.warn("[desktop-boot] legalworkServerRestart failed:", error);
            return null;
          });
          ...
          publishLegalworkServerInfo(serverInfo);
          markReady();
        };
        ...
        if (isElectronRuntime()) {
          ...
          let serverInfo = boot.legalworkServer;
          if (preferredRemoteAccess && serverInfo?.remoteAccessEnabled !== true) {
            const restarted = await legalworkServerRestart({ remoteAccessEnabled: true }).catch((error) => {
              console.warn("[desktop-boot] legalworkServerRestart failed:", error);
              return null;
            });
            if (isLegalworkServerInfoLike(restarted)) serverInfo = restarted;
          }
          publishLegalworkServerInfo(serverInfo);
          markReady();
          return;
        ...
        // current legalwork-server base URL + token and finish in <1s.
        ...
        try {
          ...
          if (engine?.running && engine.baseUrl) {
            ...
            const fresh = await legalworkServerInfo().catch(() => null) as LegalworkServerInfo | null;
            if (fresh?.baseUrl) {
              writeLegalworkServerSettings({
              ...
              try {
                window.dispatchEvent(
                  new CustomEvent("legalwork-server-settings-changed"),
                );
              } catch {
                /* ignore */
              }
        ...
        // legalwork-server and lets that server manage OpenCode.
        ...
        if (engineStartResult) {
          ...
          try {
            const freshInfo = await legalworkServerInfo() as LegalworkServerInfo | null;
            ...
            console.warn("[desktop-boot] post-engineStart legalworkServerInfo failed:", error);
          }
        }
