...
assert.equal(await run("window.__draft.from"),'alias@example.com');assert.equal(await run("window.__draft.text.includes('Alias signature')"),true);console.log('SENDER_IDENTITY_PASS');win.destroy();app.quit();}).catch(error=>{console.error(error);app.exit(1)});`);
