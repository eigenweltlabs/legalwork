...
test('actual reader reconnects safe reads after worker unavailability without mutation retry',async()=>{
 const root=await mkdtemp(join(tmpdir(),'mail-auto-render-')),appRoot=resolve(import.meta.dir,'..');let polls=0,retries=0,statusPolls=0;
 ...
 const server=Bun.serve({hostname:'127.0.0.1',port:0,fetch:request=>{
  ...
  if(path.endsWith('/status'))value={state:++statusPolls===2?'unavailable':'ready'};else if(path.endsWith('/unlock'))value={state:'ready'};
