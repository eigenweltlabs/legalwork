...
test('mailbox UI queues independent bulk actions, shows partial failure, safely undoes and confirms permanent deletion',async()=>{
 ...
 try{
  ...
  assert.equal(await run("!!document.querySelector('.mail-action-activity')"),false);await run("document.querySelector('.mail-activity-toggle').click()");await until("!!document.querySelector('.mail-action-activity')");
  ...
  await run("document.querySelector('.mail-activity-toggle').click()");
  ...
  await run("[...document.querySelector('[role=dialog]').querySelectorAll('button')].find(b=>b.textContent==='Create').click()");await until("!document.querySelector('[role=dialog]')");assert.equal(await run("!!document.querySelector('.mail-action-activity')"),false);await run("document.querySelector('.mail-activity-toggle').click()");await until("document.body.textContent.includes('Create folder')");
  fs.writeFileSync(${JSON.stringify(join(root,'mail-actions.png'))},(await win.webContents.capturePage()).toPNG());console.log('MAIL_ACTIONS_PASS');win.destroy();app.quit();}).catch(error=>{console.error(error);app.exit(1)});
  `);
  const require=createRequire(resolve(appRoot,'../desktop/package.json'));const result=await new Promise<{code:number|null;output:string}>((done,reject)=>{const child=spawn(require('electron'),[probe],{env:{...electronDisplayEnvironment(),HOME:process.env.HOME,PATH:process.env.PATH,...(process.platform==='win32'?{SystemRoot:process.env.SystemRoot,WINDIR:process.env.WINDIR}:{})},stdio:['ignore','pipe','pipe']});let output='';const timer=setTimeout(()=>{child.kill();reject(Error(output+' renderer timeout'));},20000);child.stdout.on('data',value=>output+=value);child.stderr.on('data',value=>output+=value);child.on('error',reject);child.on('close',code=>{clearTimeout(timer);done({code,output});});});if(result.code!==0)throw Error(result.output);expect(result.output).toContain('MAIL_ACTIONS_PASS');expect(mutations.filter(item=>(item.change as {kind:string}).kind==='read')).toHaveLength(2);expect(mutations.some(item=>(item.change as {kind:string}).kind==='delete')).toBe(false);expect(mutations.at(-1)).toMatchObject({change:{kind:'mailbox',operation:'create',path:'Litigation'},precondition:'gmail-mailbox-v1'});
