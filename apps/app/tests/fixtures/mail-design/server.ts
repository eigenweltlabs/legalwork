...
let mode='large',cursor=1;const drafts=new Map();const seen=new Set();
...
if(path==='/fixture/arrival'){cursor++;return Response.json({cursor},{headers});}
...
if(path.includes('/mail')){
 ...
 else if(path.endsWith('/folders'))result={items:[{id:'INBOX',name:'Inbox',kind:'folder',parentId:null,role:'inbox',mutationPrecondition:'current'},{id:'sent',name:'Sent',kind:'folder',parentId:null,mutationPrecondition:'current'},{id:'work',name:'Aster',kind:'folder',parentId:null,mutationPrecondition:'current'}],nextCursor:null};
 ...
 else if(path.endsWith('/events/query'))result={stream:'synthetic',nextCursor:cursor};
 else if(path.endsWith('/actions/query'))result={items:[],nextCursor:null};
 else if(path.endsWith('/search')){const offset=input.offset||0;result={items:eligible.slice(offset,offset+25).map(item=>({accountId:item.accountId,locator:item.locator,subject:item.subject,sender:item.metadata.from.match(/<([^>]+)>/)?.[1]??item.metadata.from,snippet:'Please review the final comments in the agreement. Revised notice period: 60 days.',date:new Date(item.receivedAt).toISOString(),hasAttachment:true})),total:eligible.length,pending:0,incomplete:0,nextOffset:offset+25<eligible.length?offset+25:null};}
 else if(path.endsWith('/agent-access'))result={grants:[],proposals:[],workspaces:[{id:workspace.id,name:workspace.name}]};
