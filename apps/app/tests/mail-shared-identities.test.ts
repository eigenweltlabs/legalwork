...
test('Settings preserves explicit shared grants and composer binds the mailbox From identity',async()=>{
 ...
 await until("document.body.textContent.includes('Mailbox access verified')");await run('window.__compose()');await until("!!document.querySelector('[aria-label=\\\"Sender email\\\"]')");assert.equal(await run("document.querySelector('[aria-label=\\\"Sender email\\\"]').readOnly"),true);assert.equal(await run("document.querySelector('[aria-label=\\\"Sender email\\\"]').value"),'team@example.com');assert.equal(await run("document.body.textContent.includes('Sending from this mailbox is not authorized')"),true);assert.equal(await run("document.body.textContent.includes('Sent Items are saved in the signed-in Microsoft account')"),true);console.log('SHARED_IDENTITY_PASS');win.destroy();app.quit();}).catch(error=>{console.error(error);app.exit(1)});`);
