...
await until("!!document.querySelector('[data-work-row]')");await run("document.querySelector('[data-work-row]').click()");await until("document.body.textContent.includes('will not resend automatically')");assert.equal(await run("[...document.querySelectorAll('button')].some(el=>el.textContent==='Retry submission')"),false);
