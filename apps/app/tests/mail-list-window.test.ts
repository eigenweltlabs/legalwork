...
test('incremental lists retain only four 250-row windows and preserve back/forward ordering',()=>{
 ...
 for(let offset=0;offset<1250;offset+=25){pages.append(Array.from({length:25},(_,i)=>offset+i));expect(pages.rows.length).toBeLessThanOrEqual(250);expect(pages.retained.length).toBeLessThanOrEqual(1000);}
