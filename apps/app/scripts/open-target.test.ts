...
describe("deriveOpenTargets", () => {
  ...
  it("extracts PowerPoint decks from assistant artifact summaries", () => {
    const targets = deriveOpenTargets([
      message("msg_1", "assistant", "Updated file: decks/legalwork-vertebrae-deck.pptx"),
    ]);
    const deck = targets.find((target) => target.value === "decks/legalwork-vertebrae-deck.pptx");
  ...
  it("extracts file paths from assistant output without requiring artifact verbs", () => {
    const targets = deriveOpenTargets([
      message("msg_1", "assistant", "Done. **Aufhebungsvertrag_Christian_Poensgen.docx**"),
    ]);

    expect(targets[0]).toMatchObject({ value: "Aufhebungsvertrag_Christian_Poensgen.docx", preview: "word", confidence: 65 });
  });

  it("extracts artifact paths from LegalWork extension call metadata", () => {
    const targets = deriveOpenTargets([
      toolMessage("msg_tool", "legalwork_extension_call", {
  ...
  it("does not turn package search results into artifacts unless the assistant mentions them", () => {
    const targets = deriveOpenTargets([
