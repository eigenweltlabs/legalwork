...
// LegalWork does not ship a built-in Google OAuth client. Operators can supply
// their own Desktop/installed-app client through env vars; per Google, that
// client secret is non-confidential and is protected here by PKCE + loopback OAuth.
const GOOGLE_WORKSPACE_DESKTOP_CLIENT_ID = "";
const GOOGLE_WORKSPACE_DESKTOP_CLIENT_SECRET = "";
const GOOGLE_WORKSPACE_CLIENT_ID_ENV = "LEGALWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_ID";
const GOOGLE_WORKSPACE_CLIENT_SECRET_ENV = "GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET";
const LEGACY_GOOGLE_WORKSPACE_CLIENT_SECRET_ENV = "LEGALWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET";
const GOOGLE_WORKSPACE_TOKEN_BROKER_URL_ENV = "LEGALWORK_GOOGLE_WORKSPACE_TOKEN_BROKER_URL";
const GOOGLE_WORKSPACE_ALLOW_PLAINTEXT_VAULT_ENV = "LEGALWORK_GOOGLE_WORKSPACE_ALLOW_PLAINTEXT_VAULT";
...
const GOOGLE_WORKSPACE_SCOPES = [
  ...
  // Read-only: search/read across the whole Drive, no writes. Restricted scope, so the
  // firm's client lists it on the consent screen + passes Google verification. (Gmail
  // is intentionally omitted — every Gmail scope is restricted and write-capable, which
  // a read-only profile shouldn't request.)
...
export const GOOGLE_WORKSPACE_EXTENSION_ACTIONS = [
  {
    ...
    description: "Check whether Google Workspace is connected and ready for LegalWork extension actions.",
  ...
  },
  {
    ...
    description: "Search files available to LegalWork through the connected Google Drive scope. With full Drive access enabled, this searches the entire Drive.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "Read a Drive file available to LegalWork by file id.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "Replace the plain text content of a Drive file available to LegalWork by file id.",
    inputSchema: {
      type: "object",
      properties: {
        fileId: { type: "string", description: "Google Drive file id." },
...
function configDir(config: ServerConfig): string {
  return dirname(config.configPath?.trim() || resolve(homedir(), ".config", "legalwork", "server.json"));
}
...
function googleWorkspaceCredentials() {
  ...
  const usingBuiltInClient = Boolean(GOOGLE_WORKSPACE_DESKTOP_CLIENT_ID) && clientId === GOOGLE_WORKSPACE_DESKTOP_CLIENT_ID;
  // If a future build ships a built-in Desktop client, only pair that bundled
  // non-confidential secret with that exact client id. A custom client_id must
  // bring its own secret via env.
  const clientSecret = process.env[GOOGLE_WORKSPACE_CLIENT_SECRET_ENV]?.trim()
    || process.env[LEGACY_GOOGLE_WORKSPACE_CLIENT_SECRET_ENV]?.trim()
    || (usingBuiltInClient ? GOOGLE_WORKSPACE_DESKTOP_CLIENT_SECRET : "");
  ...
  const customClient = !usingBuiltInClient;
...
function googleWorkspacePlainTextVaultEnabled() {
  return process.env.LEGALWORK_DEV_MODE === "1" && process.env[GOOGLE_WORKSPACE_ALLOW_PLAINTEXT_VAULT_ENV] === "1";
}
...
async function googleWorkspaceVaultKey(config: ServerConfig): Promise<Buffer> {
  const envKey = process.env.LEGALWORK_ENCRYPTION_KEY?.trim();
...
async function googleWorkspaceAccessToken(config: ServerConfig): Promise<{ record: Record<string, unknown>; accessToken: string }> {
  ...
  if (!record) throw new ApiError(400, "google_workspace_not_connected", "Connect Google Workspace in LegalWork Settings to use this tool.");
...
export async function googleWorkspaceRunScopeSmokeTest(config: ServerConfig) {
  ...
  const driveBoundary = `legalwork_${randomBytes(8).toString("hex")}`;
  const driveFile = await fetchGoogleJson("https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name,webViewLink", {
    ...
    body: multipartRelatedBody({ name: "LegalWork Google Workspace smoke test.txt", mimeType: "text/plain" }, `LegalWork Google Workspace smoke test created at ${createdAt}.`, driveBoundary),
  });
  ...
  const draft = await fetchGoogleJson("https://gmail.googleapis.com/gmail/v1/users/me/drafts", {
    method: "POST",
    headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
    body: JSON.stringify({ message: { raw: base64UrlString(gmailRawMessage({ to: [email], subject: "LegalWork Google Workspace smoke test draft", body: `This draft was created by LegalWork to verify Gmail draft access at ${createdAt}.\nLegalWork does not send this email automatically.` })) } }),
  });
  return googleWorkspaceStatusPayload(record, {
    ...
    const port = await new Promise<number>((resolvePort, reject) => {
      callbackServer = createServer(async (request, response) => {
        ...
        try {
          const flow = flows.get(flowId);
          if (!flow) {
            await finish(googleWorkspaceCallbackPage(410, "Google Workspace connection expired", "Return to LegalWork and start connection again."));
            return;
          }
          ...
          await finish(googleWorkspaceCallbackPage(200, "Google Workspace authorization received", "You can return to LegalWork while it finishes connecting."));
          try {
            ...
            flow.status = "failed";
            flow.error = `Google authorized LegalWork, but token exchange failed: ${exchangeError instanceof Error ? exchangeError.message : String(exchangeError)}`;
          }
        } catch (callbackError) {
          const flow = flows.get(flowId);
          if (flow) {
