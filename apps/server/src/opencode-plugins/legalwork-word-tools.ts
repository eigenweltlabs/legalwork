...
- An edit request that names no file is about the open document. "Rename X to Y", "change the date", "fix clause 4", "replace all X with Y" mean the document the user is looking at: go straight to the word_* tools. Do NOT grep the workspace for the text and do NOT treat it as a find-and-replace across files — that edits something the user cannot see and leaves the real document untouched.
- The workspace usually holds sidecar copies of the open document: a .md, .txt, or .pdf export with the same name. Editing one of those does not change what the user is looking at. Never satisfy a request about the document by editing a sidecar; if the user actually means one, they will name it.
