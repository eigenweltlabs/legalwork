...
// is for service credentials, not LegalWork/OpenCode runtime knobs; users who
...
// identity.
const RESERVED_PREFIXES = ["LEGALWORK_", "OPENCODE_"] as const;
const PERSISTABLE_INTERNAL_KEYS = new Set([
  "LEGALWORK_API_KEY",
  "LEGALWORK_MODELS_API_KEY",
  "LEGALWORK_INFERENCE_BASE_URL",
  "LEGALWORK_MODELS_BASE_URL",
]);
...
export function resolveDefaultEnvStorePath(): string {
  const override = (process.env.LEGALWORK_ENV_STORE ?? "").trim();
  if (override) return resolve(override);

  if (platform() === "win32") {
    const appData = (process.env.APPDATA ?? "").trim();
    const root = appData || join(homedir(), "AppData", "Roaming");
    return join(root, "legalwork", "env.json");
  }
  return join(homedir(), ".config", "legalwork", "env.json");
}
  ...
  constructor(key: string, code: "invalid_env_key" | "reserved_env_key") {
    super(
      code === "reserved_env_key"
        ? `Environment variable name is reserved for LegalWork internals: ${key}`
