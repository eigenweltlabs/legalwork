...
export async function probeWorkerExecutable(executable: WorkerExecutable): Promise<{ nodeVersion: string }> {
  ...
  const script = 'process.stdout.write(JSON.stringify({node:process.versions.node,napi:process.versions.napi,bun:Boolean(process.versions.bun)}))';
  return new Promise((resolve, reject) => {
    execFile(executable.path, ["--eval", script], {
      ...
      try {
        ...
        if (typeof value !== "object" || value === null || !("node" in value) || !("bun" in value) || !("napi" in value)
          || typeof value.node !== "string" || !/^\d+\.\d+\.\d+$/.test(value.node)
          || Number(value.node.split(".")[0]) < 22 || !Number.isInteger(Number(value.napi)) || Number(value.napi) < 10 || value.bun !== false) {
