...
| --- | --- |
| `ping` | `{pong:true}` |
| `mail.storage.status` | `{encrypted:true,schemaVersion:number,syncSupported:false}` |
...
| `mail.status` with `accountId` | `{state:"idle",syncSupported:false}` after owner-scoped account verification |
