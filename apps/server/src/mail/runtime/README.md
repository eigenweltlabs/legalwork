...
This directory implements private worker transport and the production `worker.ts` entrypoint. The production worker opens the encrypted database, migrates its schema, binds a repository to one trusted owner and serves typed, bounded local mail operations. Account-owned Gmail, Microsoft Graph and IMAP sessions synchronize encrypted messages and attachments; credential custody, OAuth refresh, local drafts, provider drafts, mailbox action journals, search and notification delivery share this worker. The authenticated server service exposes narrow routes above this private transport. `fixtures/worker.mjs` is deliberately a fault-injection test fixture; never configure it as the production worker entrypoint.
...
`initialize()` is re-read for every launch, allowing the parent to retrieve current keychain credentials after a crash. Production initialization requires `ownerId`, absolute `databasePath`, and `encryptionKey` encoded as canonical base64 of exactly 32 bytes. `ownerId` comes from the desktop/authenticated service's stable principal, never an HTTP body, query, client header or account ID. Provider credentials are held in encrypted account custody and refreshed by the account access coordinator. Startup does not accept arbitrary credential bags; trusted OAuth configuration is restored by LocalMailService for connected accounts. The optional lifecycleSuspended flag prevents background work from starting during a persistent pause. These values travel exclusively through the child's private stdin pipe. They are never command arguments, environment, diagnostics or persisted by this transport. The worker validates canonical key encoding, passes a decoded Buffer to the fail-closed encrypted opener, clears that Buffer, migrates the schema and binds `MailRepository` to the supplied owner. The supplier owns OS-vault key retrieval and its preflight; the worker owns encrypted provider refresh persistence. JavaScript strings cannot promise deterministic memory zeroization; this transport keeps no durable secret store.
...
The strict command and result unions in `protocol.ts` are the transport source of truth. See [local APIs](../../../../../docs/mail/local-drafts-actions-events.md), [draft synchronization](../../../../../docs/mail/draft-synchronization.md), and [notifications and lifecycle](../../../../../docs/mail/notifications-keyboard-lifecycle.md) for workflows. Extend the unions, validators and tests together with actual worker handlers; never add an arbitrary SQL, filesystem, network or shell proxy.
...
Connected Gmail, Graph and IMAP accounts resume automatically after service opening and supervised worker restart, preserving deliberate per-account pauses. Gmail history, Graph delta and IMAP polling reconcile subsequent changes. Background suspension closes provider sessions and aborts draft work without changing durable account pauses; resume restores eligible sessions. The reader reloads safe reads when the worker becomes ready. Transport recovery never replays a failed mutation; durable journals retain its pending, failed or uncertain outcome. Ready describes the encrypted worker, not provider reachability or successful synchronization.

This channel is private to a parent-created process and is not a server authentication boundary. The parent must authenticate/authorize public routes, validate account ownership and translate narrow domain operations; never expose the transport directly as a remote command proxy. The real worker must independently validate frames and account scope. Root integration owns build/packaging, desktop key supply, server auth/routes and application shutdown hooks. Build `worker.ts` to `dist/mail/runtime/worker.js`; native modules resolve relative to the built storage module via createRequire, so staging must preserve package resolution and unpack native artifacts. Credential updates acknowledge success only after encrypted persistence; refresh persistence is handled internally by the account access coordinator and credential repository.
