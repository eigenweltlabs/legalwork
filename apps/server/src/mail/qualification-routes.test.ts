...
test('qualification is host-only, strict, bounded and cancels a disconnected HTTP start',async()=>{
 ...
 service.qualification=async input=>{calls.push(input.action);if(input.action==='start')abort.abort();return{id,provider:'gmail',mode:'full',sampleComparison:'pending',httpStatus:null,providerReason:null,startedAt:1,finishedAt:null,state:'running',comparison:'pending',phase:'enumeration',maxMessages:2,sampleLimit:1,scope:'all_messages_including_spam_trash',providerMessages:0,localMessages:0,missingLocal:0,extraLocal:0,duplicateProviderIds:0,providerLabels:0,localLabels:0,missingLabels:0,extraLabels:0,membershipsChecked:0,metadataRetries:0,membershipMismatches:0,rawSamples:0,rawMismatches:0,attachmentSamples:0,attachmentMismatches:0,bodySamples:0,bodyMismatches:0,incompleteSamples:0,incompleteReasons:{readerNotComplete:0,storedProjectionNotComplete:0,missingRaw:0,missingBody:0,unavailableParts:0,bodyTooLarge:0,freshUnsupported:0,freshMalformed:0,freshLimit:0,freshOther:0},skippedOversizeSamples:0,downloadedSampleBytes:0,providerStable:null,localStable:null,error:null,transportCode:null,retryAfterMs:null,failedPhase:null};};
