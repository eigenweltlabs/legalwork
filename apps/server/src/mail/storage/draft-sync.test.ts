...
test("Draft synchronization and immutable recovery use actual encrypted Node storage", () => {
  ...
  try {
    ...
    const target = join(output, "mail/storage/draft-sync.node-test.mjs");
    copyFileSync(join(import.meta.dir, "draft-sync.node-test.mjs"), target);
