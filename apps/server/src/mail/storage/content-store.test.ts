...
/** Compile production TS without Bun native loading, then exercise the encrypted Node adapter. */
test("streamed encrypted content contract runs in Node", () => {
  const output = mkdtempSync(join(tmpdir(), "legalwork-mail-content-build-"));
  ...
  try {
    ...
    const target = join(output, "mail/storage/content-store.node-test.mjs");
    copyFileSync(join(import.meta.dir, "content-store.node-test.mjs"), target);
