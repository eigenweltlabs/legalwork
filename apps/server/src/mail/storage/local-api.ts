import {mailDraftPreview} from '../local-view.js';
...
export class MailLocalApiStore{
 ...
 private account(accountId:string):void{if(this.db.get('SELECT provider FROM mail_accounts WHERE id=? AND owner_id=?',[accountId,this.ownerId])?.provider==='archive')throw new MailLocalError('invalid_input');if(!this.db.get('SELECT id FROM mail_accounts WHERE id=? AND owner_id=?',[accountId,this.ownerId]))throw new MailLocalError('not_found');if(this.credentials.status(accountId).state==='disconnected')throw new MailLocalError('locked');}
 ...
 readDraft(accountId:string,supplied:MailDraftRead){return this.safe(()=>{this.account(accountId);const input=parse(mailDraftReadSchema,supplied),head=this.head(accountId,input.draftId);if(head.deleted&&!input.version)throw new MailLocalError('not_found');const version=input.version??this.version(head),content=this.snapshot(accountId,input.draftId,version);return mailDraftViewSchema.parse({id:head.id,version,updatedAt:this.db.get("SELECT updated_at FROM mail_local_draft_versions WHERE account_id=? AND draft_id=? AND generation=? AND revision=?",[accountId,head.id,version.generation,version.revision])?.updated_at,deleted:head.deleted===1,subject:content.subject,summary:mailDraftPreview(content),content});});}
 private draftCursor(accountId:string,after?:string){
  if(!after)return null;
  if(z.string().uuid().safeParse(after).success){const head=this.head(accountId,after);return{time:head.updated_at,id:head.id};}
  let value:unknown;try{value=JSON.parse(after);}catch{throw new MailLocalError('invalid_input');}
  return parse(z.object({time:z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER),id:z.string().uuid()}).strict(),value);
 }
 listDrafts(accountId:string,supplied:MailLocalPage){return this.safe(()=>{this.account(accountId);const page=parse(mailLocalPageSchema,supplied);const cursor=this.draftCursor(accountId,page.after);const rows=cursor?this.db.all('SELECT id FROM mail_local_drafts WHERE account_id=? AND deleted=0 AND (updated_at<? OR (updated_at=? AND id<?)) ORDER BY updated_at DESC,id DESC LIMIT ?',[accountId,cursor.time,cursor.time,cursor.id,page.limit+1]):this.db.all('SELECT id FROM mail_local_drafts WHERE account_id=? AND deleted=0 ORDER BY updated_at DESC,id DESC LIMIT ?',[accountId,page.limit+1]);const items=rows.slice(0,page.limit).map(row=>{const {content,...summary}=this.readDraft(accountId,{draftId:string.parse(row.id)});return mailDraftSummarySchema.parse(summary);});return{accountId,items,nextCursor:rows.length>page.limit?JSON.stringify({time:items.at(-1)?.updatedAt,id:items.at(-1)?.id}):null};});}
