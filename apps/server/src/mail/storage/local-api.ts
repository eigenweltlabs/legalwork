...
export class MailLocalApiStore{
 ...
 enqueueSubmission(accountId:string,supplied:MailSubmission){return this.safe(()=>{const input=parse(mailSubmissionSchema,supplied),intent={kind:'submission',draftId:input.draftId,version:input.version};const result=this.enqueue(accountId,'submission',input.replayKey,intent,()=>{const head=this.head(accountId,input.draftId);if(head.deleted)throw new MailLocalError('not_found');const content=this.snapshot(accountId,input.draftId,input.version);if(this.db.get('SELECT provider FROM mail_accounts WHERE id=?',[accountId])?.provider==='graph')new GraphMailboxRepository(this.db,this.ownerId).assertSender(accountId,content.from);if(!content.to.length&&!content.cc.length&&!content.bcc.length)throw new MailLocalError('invalid_input');},null);this.db.run('INSERT INTO mail_local_action_drafts(account_id,action_id,draft_id,generation,revision) VALUES(?,?,?,?,?) ON CONFLICT(account_id,action_id) DO NOTHING',[accountId,result.id,input.draftId,input.version.generation,input.version.revision]);return result;});}
