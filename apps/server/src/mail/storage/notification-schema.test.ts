...
test("Notification schema upgrade use actual encrypted Node storage", () => {
  ...
  try {
    ...
    const target = join(output, "mail/storage/notification-schema.node-test.mjs");
    copyFileSync(join(import.meta.dir, "notification-schema.node-test.mjs"), target);
