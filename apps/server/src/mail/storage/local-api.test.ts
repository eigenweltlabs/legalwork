...
test("Local drafts, intents and events use actual encrypted Node storage", () => {
  ...
  try {
    ...
    const target = join(output, "mail/storage/local-api.node-test.mjs");
    copyFileSync(join(import.meta.dir, "local-api.node-test.mjs"), target);
