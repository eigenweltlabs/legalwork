...
test('lexical, exact and structured search survive offline rebuild and encrypted reopen',async()=>fixture(async f=>{
 ...
 let search=new MailSearchStore(f.db,'owner');assert.equal(search.search({}).pending,1);assert.equal(search.rebuild({accountId:'a'}).processed,1);assert.equal(search.search({}).items[0].sender,'sender@example.test');
 ...
 const phrase={phrase:'Body private marker',accountIds:['a'],unread:true,folderId:'INBOX',afterDate:'2026-09-09T00:00:00.000Z',beforeDate:'2026-09-10T00:00:00.000Z',literal:'private',sender:'sender@example.test',filename:'prüfung-12.3.pdf',hasAttachment:true};
 assert.equal(search.search(phrase).total,1);assert.match(search.search(phrase).items[0].snippet,/private/);
 for(const changed of [{unread:false},{folderId:'missing'},{afterDate:'2026-09-09T11:00:00.000Z'},{literal:'absent'}])assert.equal(search.search({...phrase,...changed}).total,0);
