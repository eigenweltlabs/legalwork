...
test("Notification delivery use actual encrypted Node storage", () => {
  ...
  try {
    const compiled = spawnSync("pnpm", ["exec", "tsc", "--outDir", output, "--rootDir", "src", "--target", "ES2022", "--module", "NodeNext", "--moduleResolution", "NodeNext", "--strict", "--skipLibCheck", "--types", "bun-types,node",
      "src/mail/storage/notifications.ts", "src/mail/storage/local-api.ts", "src/mail/storage/search.ts", "src/mail/storage/gmail-state.ts", "src/mail/storage/mime-projection.ts", "src/mail/storage/credentials.ts", "src/mail/storage/repository.ts", "src/mail/storage/database.ts", "src/mail/storage/schema.ts", "src/mail/storage/content-store.ts"], { cwd: server, encoding: "utf8", timeout: 30000 });
    ...
    const target = join(output, "mail/storage/notifications.node-test.mjs");
    copyFileSync(join(import.meta.dir, "notifications.node-test.mjs"), target);
