...
test('upload retries, scope, verification, cleanup, pinned bytes and restart',()=>fixture(async f=>{
 ...
 const draft=f.api.saveDraft('a',{draftId:randomUUID(),expected:null,content:content({attachments:[part],inReplyTo:'<parent@example.test>',references:['<root@example.test>','<parent@example.test>']})});
 const action=f.api.enqueueSubmission('a',{replayKey:'upload-send',draftId:draft.id,version:draft.version});f.api.deleteDraft('a',{draftId:draft.id,expected:draft.version});await f.reopen();assert.equal(f.api.readAction('a',action.id).executionSupported,false);
