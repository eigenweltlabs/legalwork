...
test("Filing schema upgrade use actual encrypted Node storage", () => {
  ...
  try {
    ...
    const target = join(output, "mail/storage/filing-schema.node-test.mjs");
    copyFileSync(join(import.meta.dir, "filing-schema.node-test.mjs"), target);
