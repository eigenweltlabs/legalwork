import {mailAccountActionsSchema,mailAccountPolicySchema} from './account-policy.js';
import {z} from 'zod';
...
export const agentControlSchema=z.discriminatedUnion('action',[
 z.object({action:z.literal('account-policy'),accountId:id}).strict(),
 z.object({action:z.literal('set-account-policy'),accountId:id,expectedRevision:z.number().int().nonnegative(),actions:mailAccountActionsSchema}).strict(),
 z.object({action:z.literal('account-scope'),accountId:id}).strict(),
 ...
 z.object({action:z.literal('draft-source'),grantId:uuid,draftId:uuid,permission:z.enum(['draft','propose_send']).default('draft')}).strict(),
