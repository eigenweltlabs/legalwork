...
export const draftSyncStatusSchema=z.object({accountId:id,draftId:uuid,state:z.enum(['local','queued','syncing','synced','conflict','uncertain','error','removed']),enabled:z.boolean(),dirty:z.boolean(),revision:z.number().int().nonnegative(),remote:draftRemoteRefSchema.nullable(),remoteHash:z.string().nullable(),error:z.enum(['incomplete','unsupported','permission','remote_changed','remote_missing','dispatch_unknown','retryable','local_changed','provider_rejected']).nullable(),operation:z.enum(['upsert','delete','cleanup','adopt']),updatedAt:z.number().int().nonnegative()}).strict();
