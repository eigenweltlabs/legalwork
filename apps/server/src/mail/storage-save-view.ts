...
export const storageSaveInputSchema=z.discriminatedUnion('action',[
 z.object({action:z.literal('create'),accountId:id,snapshotId:uuid,workspaceId:id,storageId:id,rootRevision:z.string().min(1).max(4096),folderPath:z.string().max(2048),selection:z.array(size.max(100)).min(1).max(101),fresh:z.boolean().optional()}).strict(),
 z.object({action:z.literal('cancel'),accountId:id,id:uuid}).strict(),
...
export const storageSaveResultSchema=z.discriminatedUnion('action',[
 ...
 z.object({action:z.literal('cancel'),save:storageSaveSchema}),
...
export function storageSaveResultMatches(input:StorageSaveInput,result:StorageSaveResult):boolean{
 ...
 if(input.action==='create'&&result.action==='create')return result.save.accountId===input.accountId&&(input.fresh||result.save.snapshotId===input.snapshotId)&&result.save.workspaceId===input.workspaceId&&result.save.storageId===input.storageId&&result.save.rootRevision===input.rootRevision;
 if((input.action==='read'||input.action==='part'||input.action==='cancel')&&(result.action==='read'||result.action==='part'||result.action==='cancel'))return result.save.accountId===input.accountId&&result.save.id===input.id;
 return false;
}
