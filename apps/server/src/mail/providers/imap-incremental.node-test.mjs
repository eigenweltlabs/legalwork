...
test('polling reconciles flags, expunges, folder removal/addition and UIDVALIDITY while preserving originals and draft/action records',()=>fixture(async f=>{
 ...
 f.server.state.flags['INBOX:1']=[];f.server.state.messages.INBOX.push(1003);delete f.server.state.messages.Archive;f.server.state.messages.New=[2];engine.start(accountId);await until(()=>engine.status(accountId).state==='complete');assert.equal(f.db.get("SELECT is_read FROM mail_messages WHERE message_key=?",[JSON.stringify(['imap','INBOX',7,1])]).is_read,0);assert.equal(engine.status(accountId).removed,1);assert.equal(engine.status(accountId).retained,1);assert.equal(f.db.get("SELECT id FROM mail_folders WHERE account_id=? AND id='Archive'",[accountId]),undefined);assert.equal(engine.discovery(accountId).folders.some(folder=>folder.path==='Archive'),false);assert.equal(f.db.get('SELECT count(*) AS n FROM mail_local_drafts').n,1);
...
test('special archive uses server SPECIAL-USE with UID-safe MOVE; ambiguous or unavailable targets never dispatch',()=>fixture(async f=>{
 f.server.state.capabilities=['MOVE'];const c=await ready(f);
 assert.equal((await mutate(f,c,{kind:'special',operation:'trash'},'missing-trash')).providerResult,'unsupported');assert.deepEqual(f.server.state.writeCommands,[]);
 const archived=await mutate(f,c,{kind:'special',operation:'archive'},'archive-special');assert.equal(archived.state,'succeeded');assert.deepEqual(f.server.state.writeCommands,['MOVE']);assert.ok(!f.server.state.messages.INBOX.includes(1));await until(()=>c.engine.status(c.accountId).state==='complete');assert.ok(c.engine.status(c.accountId).retained>=1);
}));
