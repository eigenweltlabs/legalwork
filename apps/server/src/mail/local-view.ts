...
export const mailDraftContentSchema=z.object({subject:header,to:z.array(mailAddressSchema).max(50),cc:z.array(mailAddressSchema).max(50).default([]),bcc:z.array(mailAddressSchema).max(50).default([]),from:mailAddressSchema.nullable().default(null),senderIdentityId:z.string().min(1).max(256).nullable().default(null),senderSignature:z.string().max(4000).default(''),text:z.string().max(24000),html:z.string().max(24000).nullable().default(null),
