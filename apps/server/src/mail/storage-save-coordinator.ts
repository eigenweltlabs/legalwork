...
export class MailStorageSaveCoordinator {
 ...
 async roots(workspaceId:string,signal?:AbortSignal){const response=await this.request(this.base(workspaceId)+'/roots',{signal:signal?AbortSignal.any([signal,AbortSignal.timeout(30000)]):AbortSignal.timeout(30000)});if(!response.ok)throw new ApiError(409,'mail_storage_unavailable','Connected file storage is unavailable. Configure it in Settings.');return storageRootsViewSchema.parse(await response.json());}
 async folders(workspaceId:string,storageId:string,path:string,cursor?:string,signal?:AbortSignal){storageSaveFolder(path);const response=await this.request(this.base(workspaceId)+'/'+encodeURIComponent(storageId)+'/children?'+new URLSearchParams({path,...cursor?{cursor}:{}}),{signal:signal?AbortSignal.any([signal,AbortSignal.timeout(30000)]):AbortSignal.timeout(30000)});if(!response.ok)throw Error('Storage folder unavailable');return storageFolderViewSchema.parse(await response.json());}
