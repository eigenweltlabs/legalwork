...
export function validateMcpConfig(config: Record<string, unknown>): void {
  ...
  if (type === "remote") {
    ...
    // A templated URL like https://{instance}.highq.com/{site}/mcp parses fine via
    // `new URL` (host becomes "{instance}.highq.com") but is an unfilled placeholder.
    // The engine then POSTs the OAuth handshake to a host that can't resolve and
    // surfaces a scary "Unexpected server error". Reject it so the firm fills in its
    // instance/tenant first.
    if (/[{}]/.test(normalizedUrl)) {
      throw new ApiError(400, "invalid_mcp_config", "Remote MCP url still contains a placeholder (e.g. {instance}); fill in your instance/tenant before connecting");
    }
    try {
