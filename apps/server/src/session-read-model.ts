...
export const sessionStatusSchema = z.discriminatedUnion("type", [
  ...
  z.object({
    ...
    message: z.string(),
    next: z.number(),
    // Engine-provided action block for actionable retries (e.g. re-auth,
    // upgrade or top-up prompts). Must survive the snapshot path or the
    // app's retry banner loses its button on reload.
    action: z
      .object({
        reason: z.string(),
        provider: z.string(),
