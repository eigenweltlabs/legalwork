...
describe("validateMcpConfig", () => {
  ...
  test("rejects remote url with an unfilled placeholder", () => {
    // Regression: templated connector URLs (HighQ, SharePoint, iManage) parse via
    // `new URL` but the engine 405s on the placeholder host -> err_be730ea2.
    expect(() => validateMcpConfig({ type: "remote", url: "https://{instance}.highq.com/{site}/mcp" })).toThrow();
    expect(() => validateMcpConfig({ type: "remote", url: "https://agent365.svc.cloud.microsoft/agents/tenants/{tenant_id}/servers/mcp_SharePointRemoteServer" })).toThrow();
    expect(() => validateMcpConfig({ type: "remote", url: "https://{customer}.cloudimanage.com/mcp" })).toThrow();
  });
