...
const input = storageInputSchema.parse({ name: "OAuth verification", config: { kind: "oauth", provider: providerId, root: process.env.LEGALWORK_OAUTH_TEST_ROOT ?? "" } });
...
try {
  ...
  const local = resolve(directory, `${providerId}-download-${Date.now()}.txt`);
