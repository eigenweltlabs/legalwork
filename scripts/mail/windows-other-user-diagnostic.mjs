...
try {
 ...
 for(const variant of ['original','instrumented'])for(const [label,path] of [['short',short],['long',long]]){
  ...
  const safe=(stdout+'\n'+stderr).split(/\r?\n/).filter(line=>/^(mail_other_(os_user_denied|user_phase=[a-z-]+|user_qualification_failed(?: stage=[a-z-]+ exception=[A-Za-z0-9]+)?)|child_(exit=-?\d+ stdout_chars=\d+ stderr_chars=\d+|phase=[a-z-]+|(?:inner_)?exception=[A-Za-z0-9]+))$/.test(line));
