...
try {
 ...
 for(const environmentKind of ['native'])for(const variant of ['instrumented'])for(const workingDirectory of ['inherited','system'])for(const [label,path] of [['short',short],['long',long]]){
  ...
  const safe=(stdout+'\n'+stderr).split(/\r?\n/).filter(line=>/^(exception_(native_code|hresult)=-?[0-9]+|error_id=[A-Za-z0-9_,.+-]{1,256}|exception_file=[A-Za-z0-9_.-]{1,128}|module_probe=[A-Za-z.]+ directory=(?:True|False) manifest=(?:True|False)|mail_other_(os_user_denied|user_phase=[A-Za-z.-]+|user_qualification_failed(?: stage=[A-Za-z.-]+ exception=[A-Za-z0-9]+)?)|child_(exit=-?\d+ stdout_chars=\d+ stderr_chars=\d+|phase=[a-z-]+|(?:inner_)?exception=[A-Za-z0-9]+))$/.test(line));
  console.log(JSON.stringify({environmentKind,variant,workingDirectory,pathKind:label,pathCharacters:path.length,ok,elapsedMs:Date.now()-started,diagnostics:safe}));
 }
