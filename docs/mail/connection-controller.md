...
`cancel(connectionId)` settles an active flow as cancelled and awaits bounded listener cleanup. Terminal cancellation is idempotent. `disconnect(accountId)` first persists the owner-scoped credential disconnect using its current version, then cancels explicit reconnect flows for that account and awaits cleanup. Already-disconnected accounts retain their visible state but still rotate the credential version on every call, fencing reconnects begun in other controllers/processes; unconfigured/foreign/missing accounts fail account_not_found. Metadata/content remain intact and no provider revocation occurs.
...
The Bun harness builds the production controller and runs 16 tests in actual Node against the real encrypted SQLite adapter/schema v4. Tests cover successful encrypted persistence and secret-free status, deterministic duplicates, explicit reconnect, immutable identity/client binding, disconnect/CAS races (including controller disconnect during consent/identity before cleanup and repeated disconnect fencing another controller), foreign/unconfigured account rejection, atomic rollback on an injected database trigger failure, consent/identity cancellation, late listener startup, close, expiration/capacity/retention, permission checks, absent refresh credentials and redacted failures. Provider OAuth/identity are trusted synthetic seams; no browser or live provider HTTP was used.
