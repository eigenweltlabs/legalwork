...
Initial policy: retain locally downloaded content after a remote deletion for a configurable 30-day recovery window, unless the user explicitly chooses immediate local purge. Make this policy visible at onboarding; firm retention policy can override it. This is product recovery behavior, not a legal retention guarantee. Explicit local purge requires a clear scope and removes only unreferenced content; a matter copy is not silently purged with an account.
