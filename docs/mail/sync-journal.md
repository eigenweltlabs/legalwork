...
`status(scope)` returns the checkpoint and queued/running/retry/succeeded/failed counts. **Job success does not attest to downloaded bytes.** The content repository's independently verified publication/manifest completeness remains authoritative. Explicit empty discovery requires a committed page with discoveryComplete=true and no jobs. Child body/attachment discovery after enumeration uses the fenced completion API below; no executor or network fetching is shipped here.

## Atomic raw-message followups

`succeedWithFollowups(accountId, jobId, leaseToken, jobs, persistResult)` preserves the original `succeed` API, which delegates with an empty followup list. Up to 1,000 validated child descriptors are accepted. Only a running **raw** parent may discover nonempty followups; children must be body/attachment jobs for the exact same canonical provider message identity. Attachment part IDs remain required. Account and generation are inherited, not caller-selected; extra fields, another provider/message, raw/self jobs and followups from body/attachment jobs are rejected. This restricted raw-to-part relationship cannot create job cycles.

Inside one encrypted transaction the API validates the current lease, applies the revocable synchronous metadata writer, inserts/deduplicates children, links them to every scope sharing the raw parent and marks the parent succeeded. It rechecks the lease after all writes. Callback, insertion, link or lease failure rolls everything back. Scope discoveryComplete does not prohibit this operation and neither cursor nor revision changes. A finished enumeration can therefore correctly show queued child work while the parent is succeeded.

A newly inserted child inherits the parent's **persisted** retry policy, even when the journal was reopened with different defaults. Existing children retain their state, attempt budget, availability and lease: succeeded jobs are not requeued, failed jobs remain visibly failed, running jobs are not stolen. A repeated parent completion is rejected as stale. Reprocessing changed content requires a deliberate new generation or a separately reviewed reset policy.

The documented dependency closure is all body/attachment jobs for one account/message/generation, whether initially planned by a page or discovered by the raw completion. When a later page adds that raw job to another label/scope, the existing closure is linked to that scope too. Likewise, page-discovered parts join existing raw scopes. No extra dependency graph/schema is required for this restricted relationship. This prevents a later scope from showing only a succeeded parent while its parts remain pending elsewhere. Different accounts/generations and unrelated message identities never join this closure.

Parent success and attachment-enumeration metadata are still not proof of byte publication. The caller must persist verified content using the existing content store before reporting content completeness. The callbacks cannot fetch network data or safely use independently captured SQL handles.
...
The Bun launcher strictly compiles the production modules and launches Node's test runner with the real encrypted adapter. Eighteen Node tests cover page replay/dedupe, account/provider isolation, raw/body/attachment identities, metadata/FK rollback, 1,000-job bound, claim/reclaim bounds, thenable rollback and delayed-writer rejection, fenced renewal/completion, persisted backoff/max attempts, terminal outcomes, explicit empty enumeration and v2 migration failure/retry. SIGKILL tests terminate the sole open writer inside metadata persistence, immediately after page commit and after claim; fresh processes recover an all-or-nothing checkpoint/job set and fence a killed worker's old lease. Synthetic metadata is absent from the physical encrypted database bytes.

Executed on macOS arm64 with Node24.11.0 and better-sqlite3-multiple-ciphers13.0.3. Seven additional followup cases cover closed/later shared scopes, inherited policy across reopen, terminal dedupe, identity/self/cycle rejection, stale/replaced/expired leases, injected child-insert failure, thenable rollback and SIGKILL immediately before/after atomic child completion. No real mailbox, provider download, byte-completion, 100k workload, network retry, hardware power-loss, disk-full or multi-writer performance claim follows from these tests. No provider cursor format is interpreted or sent to a network endpoint by this module.


## Executor fences and provider retry delays

`assertLease(accountId, jobId, token)` checks owner scope and the current unexpired durable lease. `readJob` is an internal bounded lookup for the executor's persisted outcome. `fail` accepts an optional nonnegative provider retry delay; the next attempt is never earlier than either persisted exponential backoff or the provider's requested minimum. Long delays survive reopen; arithmetic saturates at the maximum safe timestamp instead of shortening provider advice. These primitives do not independently execute a provider request.
