...
The checked-in `scripts/mail/verify-built-application.mjs` reproduces that check against a built `.app` on macOS or the unpacked application directory on Windows. It uses the packaged executable and archive, bounded private stdin/stdout, synthetic data and a disposable directory. After all mail tickets are complete, manually run the final platform workflow to build and verify all three required targets with publication disabled. Branch pushes and pull requests do not trigger this full qualification workflow; ongoing ticket work uses feature-specific tests. Example after building on Apple Silicon:
