...
EIG-139 owns functional conversation qualification. Its visible 20-open, plain/HTML, idle preservation, explicit unread and one-shot initial content recovery checks passed in this instance; its targeted extended-thread check then passed 25 → 50 → 55 messages across polling, with the final cursor exhausted and one expanded frame, after EIG-139 fixed StrictMode load ownership. Exact phase evidence is retained by that owner. The affected standalone Settings/onboarding tests were adapted statically for shared controls and will run in the final remote app gate; no extra local Electron test process was launched.
...
After root review, the sole synthetic Electron window and its API/Vite processes were closed. Ports 5482/5483/5484 and owned profile helpers were verified absent; evidence and worktrees remain.
