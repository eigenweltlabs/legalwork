...
Optional S3 request headers:

- Local and Team S3 forms accept one `Name: value` per line. Headers use the secret store, stay redacted in settings/agent metadata, preserve omitted values on edits, and support explicit removal. Team values are included in the platform's encrypted configuration. The SDK applies headers before signing every S3 request, including search, reads, streamed writes and retries.
- Duplicate/malformed headers and overrides of SDK-managed signing, transport or conditional-save headers are rejected. Other providers are unchanged.
