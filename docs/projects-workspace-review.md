...
2. Create a named project with or without adding a source folder. The modal has one optional Add action and no storage-mode dropdown. Leaving the source empty, cancelling the picker, or removing a selection uses the default location automatically. Desktop defaults use the OS-resolved Documents folder: `LegalWork/Projects/<project name>`. Development builds use `LegalWork Dev/Projects` separately. Electron resolves the native Documents location before any engine HOME override, including Windows redirected/OneDrive/UNC locations. `LEGALWORK_PROJECTS_DIR` can override the root with an absolute path; headless servers retain their home-based default. A selected folder is used directly. Same-named default projects receive separate folders. Existing documents remain in place.
...
- The project identity header follows the supplied reference: the name with small pencil and favorite-star actions above a rounded New Chat button and matching note/task icon buttons. Note creation reuses the existing note dialog/editor. Task creation reuses the standard task dialog and assigns the current project automatically. After creation, the modal closes and the task list refreshes without selecting the new task or opening a viewer tab; this also applies to the project and global Tasks lists. Clicking a task still opens its existing detail view. Creation behavior was checked in all three entry points with isolated fixture data, and app type check passed.
