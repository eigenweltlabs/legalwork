...
## Electron rendering follow-up

The macOS sidebar layout applied traffic-light padding to every descendant `header`. Collapsing the navigation therefore added 112 px of left padding to Univer's ribbon and formula bar, producing the blank white block. The selector now targets only `.lw-session-header`. This also keeps the spacing rule out of embedded editor headers and terminal headers.

The sheet footer now responds to panel width: it hides the zoom slider in compact panels, truncates long sheet labels at the end, and scrolls the active tab into view when its container resizes. The zoom buttons remain available. Column/row resizing and frozen-pane changes now mark the draft dirty and patch the corresponding worksheet XML on save. Sheet insertion/removal and other reference-changing operations remain blocked.

Verified in the actual Electron dev app through its existing page-level CDP endpoint, without browser launch/navigation commands: collapsed-navigation headers have zero added padding; the active tab remains visible in a 320 px panel; a dragged column changed from 103 px to 181 px, reported unsaved state, saved successfully, and retained its width after closing/reopening the test workbook. Formulas and chart parts survived. The user's checklist was reopened after testing; persistence checks used a separate synthetic workbook. App typecheck/build and 21 focused tests pass, including dimension/frozen-pane round trips and preservation of adjacent column metadata.

The earlier inspection crash was a native Electron `EXC_BAD_ACCESS` / `SIGSEGV` in `CrBrowserMain` at 13:00:33, one second after Playwright CLI's browser-level attachment. Timing strongly implicates that attachment; the unsymbolicated native report does not establish the precise internal fault. The app was restarted, and subsequent checks used page-level commands without reproducing the crash.

![Electron editor after save and reopen](./office-electron-rendering.png)
